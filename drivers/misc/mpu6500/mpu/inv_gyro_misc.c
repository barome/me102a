...
//#define DEF_SELFTEST_GYRO_FULL_SCALE         (3)//+-2000dps
//#define DEF_SELFTEST_ACCL_FULL_SCALE         (0)//+-2g
#define DEF_SELFTEST_GYRO_FULL_SCALE         (0)//+-250dps
#define DEF_SELFTEST_ACCL_FULL_SCALE         (2)//+-8g
/* wait time before collecting data */
#define DEF_GYRO_WAIT_TIME          (30)//(50) //note: 512 byte/12 byte ~= 42 (FIFO buf size maybe only 512 byte, accel + gyro = 6 byte)
		     ...
		     unsigned int len, unsigned char const *data)
{
	...
	msgs[0].scl_rate = MPU6500_SCL_RATE;
		    ...
		    unsigned int len, unsigned char *data)
{
	...
	msgs[1].buf = data;
	msgs[1].len = len;	
	msgs[1].scl_rate = MPU6500_SCL_RATE;
...
/*
printk("inv_do_test, fsr=%d, gyro_sens=%d, accl_fs=%d, sample_rate=%d\n", test_setup.fsr, test_setup.gyro_sens, test_setup.accl_fs, test_setup.sample_rate);
printk("inv_do_test, self_test_flag=%d, fifo_count=%d, packet_size=%d, packet_count=%d\n", self_test_flag, fifo_count, packet_size, packet_count);
{
    unsigned char data[6], ind;
    ind=0;
    result = inv_i2c_read(st, reg->raw_accl, 6, data);
    printk("accel raw, %d, %d, %d\n",
        (short)((data[ind]<<8)|data[ind+1]),
        (short)((data[ind+2]<<8)|data[ind+3]),
        (short)((data[ind+4]<<8)|data[ind+5]));
    ind=0;
...
//#define accel_sens  (0x20000000/(0x00010000))//8192
