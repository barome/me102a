...
static int hym8563_i2c_read_alarm(struct i2c_client *client, struct rtc_wkalrm  *tm)
{
	...
	if(!(regs[0x00]&0x80))
	tm->time.tm_min = bcd2bin(regs[0x00]) ;
	if(!(regs[0x01]&0x80))
	tm->time.tm_hour = bcd2bin(regs[0x01]) ;
	if(!(regs[0x02]&0x80))
	tm->time.tm_mday = bcd2bin(regs[0x02]) ;
	if(!(regs[0x03]&0x80))
	tm->time.tm_wday = bcd2bin(regs[0x03]) ;
	
	regs[0] = 0x0;
	hym8563_i2c_read_regs(client, RTC_CTL2, regs, 1);
	tm->enabled = regs[0]&AIE;
...
static int hym8563_i2c_set_alarm(struct i2c_client *client, struct rtc_wkalrm  *alarm)
{
	//struct i2c_client *client = to_i2c_client(dev);
	struct hym8563 *hym8563 = i2c_get_clientdata(client);
	...
	DBG("%4d-%02d-%02d(%d) %02d:%02d:%02d enabled %d\n",
		1900 + tm->tm_year, tm->tm_mon + 1, tm->tm_mday, tm->tm_wday,
...
{		
	return hym8563_i2c_set_alarm(to_i2c_client(dev),alarm);	
}

static void hym8563_alarm_test(struct i2c_client *client ,struct rtc_time rtc_alarm_rtc_time)
{
	...
	hym8563_i2c_set_alarm(client,&rtc_alarm);	
	data = hym8563_i2c_read_alarm(client,&tm);
	DBG("set alarm  - rtc %02d:%02d:%02d %02d/%02d/%04d week=%02d\n",
	...
	{	  
        	msleep(10000);
		hym8563_i2c_read_regs(client, RTC_CTL1, &data, 1);
		DBG("-----------------------------------------------0x%0x\n",data); 
...
static int __devinit hym8563_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	printk("RTC-Boot-Time: %4d-%02d-%02d(%d) %02d:%02d:%02d\n",1900 + tm_read.tm_year, tm_read.tm_mon + 1, tm_read.tm_mday, tm_read.tm_wday,tm_read.tm_hour, tm_read.tm_min, tm_read.tm_sec); //add by sherwin for reboot agen
	...
	while(1)
	{
	 	hym8563_read_datetime(client, &tm);
        	hym8563_alarm_test(client, tm);
		msleep(200);
	}
	#endif
