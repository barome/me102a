...
int rknand_read_boot_para(char data)
{
	char buf[512];
	int ret=0;
	int LBA = 7*1024;
	...
	if(sector && gpNandInfo->ftl_write){
		ret = gpNandInfo->ftl_write(LBA, sector, buf,0);
	}else{
		printk("Function of ftl_write does not work well now! \n");
	}
...
EXPORT_SYMBOL(rknand_read_boot_para);
