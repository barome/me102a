 ...
 */
_sys_u8_ ConfigGpioFunction(SystemDataType *data, _sys_u8_ setting)
{
	_sys_u8_ u8Temp;
	...
	{
	  u8Temp = data->ggbParameter->alarm_timer;
		API_I2C_SingleWrite(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_TIMER, u8Temp);	//set alarm watch dog timer
		u8Temp = data->ggbParameter->clkDivA;			//0x90
		API_I2C_SingleWrite(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_CLK_DIVA, u8Temp);
		u8Temp = data->ggbParameter->clkDivB;			//0x91
		API_I2C_SingleWrite(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_CLK_DIVB, u8Temp);
		gpioSelData = 1;
	}
	...
	{
		API_I2C_SingleRead(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_ITNR_CTRL_C, &u8Temp);		//write Reg9d
		u8Temp &= 0xf3;
		u8Temp|= (data->ggbParameter->pwm_timer << 2);
		API_I2C_SingleWrite(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_ITNR_CTRL_C, u8Temp);		//set PWM output cycle
		gpioSelData = 4;
		API_I2C_SingleRead(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_INTR_CTRL_B, &u8Temp);		//write Reg9c
		u8Temp |= 0x40;
		API_I2C_SingleWrite(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_INTR_CTRL_B, u8Temp);		//write Reg9C
	}
	return (gpioSelData);
}
...
void ConfigureGpio(SystemDataType *data)
{
  ...
  tmp8 = tmp8 | (ConfigGpioFunction(data, data->ggbParameter->gpio1) << 2);
  tmp8 = tmp8 | (ConfigGpioFunction(data, data->ggbParameter->gpio2) << 5);
...
#define BACKUP_TIME_BYTE3       (REG_TIMER)
#define BACKUP_TIME_BYTE2       (REG_CLK_DIVA)
#define BACKUP_NAC_HIGH         (REG_INTR_CTRL_A)
#define BACKUP_NAC_LOW          (REG_OTP_CTRL)
#define BACKUP_LMD_HIGH         (REG_INTR_CTRL_D)
#define BACKUP_LMD_LOW          (REG_CBC32_HIGH)
#define BACKUP_TABLE_UPDATE_IDX (REG_CLK_DIVB)
#define BACKUP_DELTA_CAP_HIGH   (REG_CBC32_LOW)
#define BACKUP_DELTA_CAP_LOW    (REG_ALARM_EN)
#define BACKUP_ADC1_CONV_TIME   (REG_CLK_DIVB)
#define BACKUP_EDVF_FLAG        (REG_CLK_DIVB)
...
void UpiLoadBatInfoFromIC(SystemDataType *data)
{
  ...
  *(u8Ptr + 3) = u8Temp;
  ...
  //Load the NAC    
  API_I2C_SingleRead(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, BACKUP_NAC_HIGH, &u8Temp);   
  u8TempHigh = u8Temp  >> 2;  
  API_I2C_SingleRead(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, BACKUP_NAC_LOW, &u8Temp);  
  data->rmFromIC = ((_sys_u16_)u8TempHigh*256) + (_sys_u16_)u8Temp;

  // Load LMD  
  API_I2C_SingleRead(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, BACKUP_LMD_HIGH, &u8Temp);   
  u8TempHigh = u8Temp & 0x3f;   
  API_I2C_SingleRead(SECURITY, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, BACKUP_LMD_LOW, &u8Temp);  
  data->fccFromIC = ((_sys_u16_)u8TempHigh *256) + (_sys_u16_)(u8Temp);
  ...
  u16Temp = (_sys_u16_)u8TempHigh;
  u16Temp = u16Temp*256;
  API_I2C_SingleRead(NORMAL, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, BACKUP_DELTA_CAP_LOW, &u8Temp);
  u8Temp = (u8Temp & 0xf0) | ((u8Temp & 0x07) << 1);
  u16Temp = u16Temp + u8Temp;
  data->deltaCapFromIC = (_sys_s16_)u16Temp;
  ...
  else
  {
    ...
    u32Temp = u32Temp*100/data->fccFromIC;
...
void UpiUpdateBatInfoFromIC(SystemDataType *data, _sys_s16_ deltaQ)
{
  ...
  {
    tmp32 = tmp32*100/data->fccFromIC;
...
void UpiSaveBatInfoTOIC(SystemDataType *data)
{
  ...
  API_I2C_SingleRead(NORMAL, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, BACKUP_DELTA_CAP_LOW, &u8Temp1);
  u8Temp1 = u8Temp1 & ALARM_EN_DECIMATE_RST;
  u8Temp1 = u8Temp1 | (u16Temp & 0x00F0) | ((u16Temp & 0x000E) >> 1);
