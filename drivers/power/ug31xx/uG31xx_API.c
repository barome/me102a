...
#else
  #if defined(uG31xx_OS_ANDROID)
    /* cwz: add for close debug information */
    __weak void ug31_printk(int level, const char *fmt, ...) {}
  #endif  ///< end of defined(uG31xx_OS_ANDROID)
...
//API Call to return all the alarm Status
//API Call to return RID
GGSTATUS upiGG_GetAlarmStatus(char *pObj, ALARM_STATUS* alarmStatus)
{
	_upi_u8_ status[3];
  ...
  pUg31xx = (struct ug31xx_data *)pObj;
	...
	if(API_I2C_Read(NORMAL, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, 0x12, 3, &status[0]) != I2C_SUCESS)
  { 
		return(UG_READ_DEVICE_ALARM_FAIL);
  }
	else
  {
		memcpy(alarmStatus,status,3);
...
GGSTATUS upiGG_ReadAllRegister(char *pObj, GG_USER_REG* pUserReg, GG_USER2_REG* pUser2Reg)
{
  ...
  pUg31xx = (struct ug31xx_data *)pObj;
	...
	if(!API_I2C_Read(NORMAL, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, 0, 16, &pUserReg->regMode))
  {
		return UG_READ_REG_FAIL;
  } 
	else
  {
		if(!API_I2C_Read(NORMAL, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, 0x15, 2, &pUserReg->regAlarmEnable))
    {
			return UG_READ_REG_FAIL;
    }  
		if(!API_I2C_Read(NORMAL, UG31XX_I2C_HIGH_SPEED_MODE, UG31XX_I2C_TEM_BITS_MODE, REG_VBAT2_LOW, sizeof(GG_USER2_REG), (_upi_u8_* )&pUser2Reg->regVbat2))		//read
    {
			return UG_READ_REG_FAIL;
    }  
		else
...
GGSTATUS upiGG_ReadDeviceInfo(char *pObj, GG_DEVICE_INFO* pExtDeviceInfo)
{
  ...
  pUg31xx = (struct ug31xx_data *)pObj;
	...
	if(upiGG_ReadAllRegister(pObj, &pUg31xx->userReg, &pUg31xx->user2Reg) == UG_READ_REG_FAIL)
	{
  ...
  if(pUg31xx->otpData.productType != UG31XX_PRODUCT_TYPE_0)
  {
	...
	pUg31xx->measData.ggbParameter = &pUg31xx->cellParameter;
...
#define MAX_DELTA_RSOC_THRESHOLD_FOR_TABLE      (5)
#define MIN_DELTA_RSOC_THRESHOLD_FOR_TABLE      (-5)

// Read GGB file and initial
#ifdef uG31xx_OS_WINDOWS
GGSTATUS upiGG_Initial(char **pObj,const wchar_t* GGBFilename,const wchar_t* OtpFileName, const wchar_t* BackupFileName)
#elif defined(uG31xx_OS_ANDROID)
...
GGSTATUS upiGG_Wakeup(char *pObj)
{
	GGSTATUS Status = UG_READ_DEVICE_INFO_SUCCESS;
