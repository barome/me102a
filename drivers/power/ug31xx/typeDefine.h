...
typedef struct CELL_PARAMETER
{
  ...
  _upi_u8_ fw_ver;            //CellParameter struct version
...
typedef struct _GG_USER_REG{
	_upi_u8_ 	regMode;				//0x00				
	_upi_u8_ 	regCtrl1;				//0x01
	_upi_s16_	regCharge;				//0x02
	_upi_u16_	regCounter;				//0x04
	_upi_s16_	regCurrentAve;			//0x06 AVE current
	_upi_s16_	regVbat1Ave;			//0x08 AVE vBat1
	_upi_u16_	regITAve;				//0x0a AVE IT

	_upi_s16_ regOffsetCurrentAve;	//0x0c AVE offset current
	_upi_u16_ regETAve;				//0x0ezz``1
	_upi_u16_ regRidAve;				//0x10
	_upi_u8_  regAlarm1Status;		//0x12
	_upi_u8_  regAlarm2Status;		//0x13
	_upi_u8_  regIntrStatus;			//0x14
	_upi_u8_  regAlarmEnable;			//0x15
    _upi_u8_  regCtrl2;				//0x16
#if defined(uG31xx_OS_ANDROID)
