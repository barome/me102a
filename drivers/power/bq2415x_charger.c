...
/* 1: bq2415x charger detect use thread */
#define BQ2415X_CHARGER_DETECT_THREAD	1

/* timeout for resetting chip timer */
#define BQ2415X_TIMER_TIMEOUT		20
...
#if BQ2415X_CHARGER_DETECT_THREAD
...
static struct bq2415x_device *gp_bq24155 = NULL;
static int g_charger_mode = -1;
static int suspend_flag=1;//ward_du
#define BQ2415X_SPEED			300 * 1000

/* 
 * rockchip get usb charger source function
 * 0: No charging , 1: USB, 2: DC, 3: unknow
 */
extern int dwc_vbus_status(void);
extern int dwc_otg_check_dpdm(void);
extern int ug31xx_cable_callback(unsigned usb_cable_state);
extern u16 bq2415x_update_battery_voltage(void);//ward_du
enum {
	NO_CABLE = 0,
 	USB_PC_CABLE = 1,
 	AC_ADAPTER_CABLE = 3
...
/**** wake_lock ****/
void bq2415x_ac_lock(struct bq2415x_device *bq)//ward_du
{
	unsigned long		flags;
	local_irq_save(flags);
	wake_lock(&bq->wake_lock);
	local_irq_restore(flags);

}

void bq2415x_ac_unlock(struct bq2415x_device *bq)//ward_du
{
	unsigned long		flags;
	local_irq_save(flags);
	wake_unlock(&bq->wake_lock);
	local_irq_restore(flags);
}
...
static int bq2415x_i2c_read(struct bq2415x_device *bq, u8 reg)
{
	...
	msg[0].scl_rate = BQ2415X_SPEED;

	msg[1].addr = client->addr;
	msg[1].flags = I2C_M_RD;
	msg[1].buf = &val;
	msg[1].len = sizeof(val);
	msg[1].scl_rate = BQ2415X_SPEED;
				...
				enum bq2415x_command command)
{
	...
	//DBG("In the Funtion %s, command = %d.\n", __FUNCTION__, command);
...
int bq2415x_charging_status()
{
	return bq2415x_exec_command(gp_bq24155, BQ2415X_CHARGE_STATUS);
}
EXPORT_SYMBOL(bq2415x_charging_status);
...
{
	DBG("In the Funtion %s.\n", __FUNCTION__);
	bq2415x_i2c_write(bq, 0x06, 0x70);//ward_du
...
static int bq2415x_set_current_limit(struct bq2415x_device *bq, int mA)
{
	...
	DBG("In the Funtion %s, current limit = %d mA.\n", __FUNCTION__, mA);
...
static int bq2415x_set_weak_battery_voltage(struct bq2415x_device *bq, int mV)
{
	...
	DBG("In the Funtion %s, weak battery voltage = %d mV.\n", __FUNCTION__, mV);
						  ...
						  int mV)
{
	...
	DBG("In the Funtion %s. battery regulation voltage = %d mV.\n", __FUNCTION__, mV);
...
static int bq2415x_set_charge_current(struct bq2415x_device *bq, int mA)
{
	...
	DBG("In the Funtion %s, charge current = %d mA.\n", __FUNCTION__, mA);
...
static int bq2415x_set_termination_current(struct bq2415x_device *bq, int mA)
{
	...
	DBG("In the Funtion %s, termination current = %d mA.\n", __FUNCTION__, mA);
...
static int bq2415x_set_defaults(struct bq2415x_device *bq)
{
	...
	bq2415x_i2c_write(bq, 0x04, 0x3b);//ward_du
	bq2415x_i2c_write(bq, 0x05, 0x04);//ward_du
...
static int bq2415x_set_mode(struct bq2415x_device *bq, enum bq2415x_mode mode)
{
	...
	case BQ2415X_MODE_OFF:
		...
		bq2415x_ac_unlock(bq);//ward_du
		ret = bq2415x_set_current_limit(bq, 100);
		#if defined (CONFIG_UPI_BATTERY)
		ug31xx_cable_callback(NO_CABLE);		//	PB TEST 20130321
		#endif
	...
	case BQ2415X_MODE_HOST_CHARGER:
		...
		bq2415x_ac_lock(bq);//ward_du
		bq2415x_i2c_write(bq, 0x04, 0x03);//ward_du
		ret = bq2415x_set_current_limit(bq, 500);
		#if defined (CONFIG_UPI_BATTERY)
		ug31xx_cable_callback(USB_PC_CABLE);		//	PB TEST 20130321
		#endif
		break;
	case BQ2415X_MODE_DEDICATED_CHARGER:
		...
		bq2415x_ac_lock(bq);//ward_du
		bq2415x_i2c_write(bq, 0x04, 0x3b);//ward_du
		ret = bq2415x_set_current_limit(bq, 1250);//ward_du
		#if defined (CONFIG_UPI_BATTERY)
		ug31xx_cable_callback(AC_ADAPTER_CABLE);		//	PB TEST 20130321
		#endif
		break;
	...
	if (charger)
		{
			...
			bq2415x_exec_command(bq, BQ2415X_CHARGE_TERMINATION_ENABLE);//ward_du
		}
	else if (boost)
...
/* 
 * bq2415x power supply charger status change 
 * status = -1: this funtion is called by system poweron.
 * status =  0: this funtion is called by bvalid interrupt.
 * status =  1: this funtion is called by usb vbus check timer.
*/

void bq2415x_charger_status(int status)
{
#if BQ2415X_CHARGER_DETECT_THREAD
	if (status > -1) {
		int pc_or_charger = -1;
		...
		if (g_charger_mode != pc_or_charger) {
			...
			DBG("In the Funtion %s, charger status change, pc_or_charger = %d.\n", __FUNCTION__, pc_or_charger);
			wake_up_process(gp_bq24155->detect_task);
		}
	}
#endif
}
EXPORT_SYMBOL(bq2415x_charger_status);


/* bq2415x power on charger detect */
void bq2415x_poweron_charger_detect(void)
{
/* 
 * 0: No charging , 1: USB, 2: DC, 3: unknow
 */
	...
	g_charger_mode = pc_or_charger;
	DBG("In the funtion %s, pc_or_charger = %d.\n", __FUNCTION__, pc_or_charger);
	if (pc_or_charger == 2) {
		bq2415x_set_mode(gp_bq24155, BQ2415X_MODE_DEDICATED_CHARGER);
	} else if (pc_or_charger == 1) {
		bq2415x_set_mode(gp_bq24155, BQ2415X_MODE_HOST_CHARGER);
	} else {
		bq2415x_set_mode(gp_bq24155, BQ2415X_MODE_OFF);
	}
}

/**** thread functions ****/
#if BQ2415X_CHARGER_DETECT_THREAD
/* bq2415x power supply charger detect thread */
static int bq2415x_charger_detect_thread(void *unused)
{
/* 
 * 0: No charging , 1: USB, 2: DC, 3: unknow
 */
...
static void bq2415x_timer_work(struct work_struct *work)
{
	...
	#if defined(CONFIG_AC_SUSPEND_MODE)
	for(i=0;i<0x07;i++)
		{
			printk("ward_du:reg%d,is :%x\n",i,bq2415x_i2c_read(gp_bq24155,i));//ward_du
		}
	#endif
}
					     ...
					     union power_supply_propval *val)
{
	...
	//DBG("In the Funtion %s, property = %d.\n", __FUNCTION__, psp);
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		DBG("In the Funtion %s, psp = POWER_SUPPLY_PROP_STATUS, ret = %d.\n", __FUNCTION__, ret);
		break;
...
int  get_bq24155_status(void)
{
	int ret;
	int status;
	ret = bq2415x_exec_command(gp_bq24155, BQ2415X_CHARGE_STATUS);
	if (ret < 0)
		return ret;
	else if (ret == 0) /* Ready */
				      ...
				      size_t count)
{
	...
	if (strncmp(buf, "auto", 4) == 0) {
		...
		if (bq->automode > 0)
			bq->automode = 0;
		if((g_charger_mode!=0)&&(g_charger_mode!=-1))
		{
			mode = BQ2415X_MODE_DEDICATED_CHARGER;
			 suspend_flag=1;//ward_du
		}
		else 
		{
			mode = BQ2415X_MODE_OFF;
					    ...
					    char *buf)
{
	...
	ret += bq2415x_sysfs_print_reg(bq, 0x05, buf+ret);
	ret += bq2415x_sysfs_print_reg(bq, 0x06, buf+ret);
	return ret;
}
...
static DEVICE_ATTR(ac_suspend_mode, S_IRWXU | S_IRWXG |S_IRWXO ,
		bq2415x_sysfs_show_ac_suspend_mode, bq2415x_sysfs_set_ac_suspend_mode);
static DEVICE_ATTR(mode, S_IRWXU |S_IRWXG |S_IRWXO,
			 ...
			 const struct i2c_device_id *id)
{
	...
	wake_lock_init(&bq->wake_lock, WAKE_LOCK_SUSPEND,"bq2415x_ac");//ward_du
	bq2415x_reset_chip(bq);
...
#if BQ2415X_CHARGER_DETECT_THREAD
	/* create charger detect thread */
	bq->detect_task = kthread_run(bq2415x_charger_detect_thread, NULL, "bq2415x_detect");
	if (!IS_ERR(bq->detect_task)) {
		struct sched_param param = { .sched_priority = MAX_RT_PRIO - 1 };
		sched_setscheduler_nocheck(bq->detect_task, SCHED_RR, &param);
	}
#endif

	bq2415x_poweron_charger_detect();

	dev_info(bq->dev, "driver registered\n");
	DBG("Funtion bq2415x_probe OK.\n");

	#if defined (CONFIG_UPI_BATTERY)
	if(bq2415x_update_battery_voltage()<3400)//ward_du
	{
		printk("ward_du:In function %s the battery voltage is below 3400mv!\n",__FUNCTION__);
		while(bq2415x_update_battery_voltage()<3600)
			{
...
//	mutex_unlock(&bq2415x_id_mutex);
	...
	DBG("Funtion bq2415x_probe ERROR.\n");
	return ret;
}
...
/* cwz: add suspend and resume code. */
static int bq2415x_suspend(struct i2c_client *client, pm_message_t state)
{
	bq2415x_set_autotimer(gp_bq24155, 0);//ward_du
	return 0;
}

static int bq2415x_resume(struct i2c_client *client)
{

#if BQ2415X_CHARGER_DETECT_THREAD
	int pc_or_charger = -1;
	if(suspend_flag==0) pc_or_charger=0;
	else pc_or_charger = dwc_vbus_status();
	DBG("In the funtion %s, pc_or_charger = %d.\n", __FUNCTION__, pc_or_charger);
	if (pc_or_charger == 2) {
		bq2415x_set_mode(gp_bq24155, BQ2415X_MODE_DEDICATED_CHARGER);
	} else if (pc_or_charger == 1) {
	...
	bq2415x_set_autotimer(gp_bq24155, 1);//ward_du
#endif
	return 0;
}
...
static int __init bq24155_charger_init(void)
{
	int ret;
	...
	if (ret)
		printk(KERN_ERR "Unable to register BQ2415x driver\n");
	
	return ret;
}
subsys_initcall_sync(bq24155_charger_init);

static void __exit bq24155_charger_exit(void)
{
	i2c_del_driver(&bq2415x_driver);
}
module_exit(bq24155_charger_exit);
