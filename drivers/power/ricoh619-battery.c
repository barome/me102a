...
#define RICOH619_MAX_ADP_CHARGE_CURRENT  1400
#define RICOH619_MAX_USB_CHARGE_CURRENT   500
#define RICOH619_SET_ADP_CHARGE_CURRENT	  1400	/* mA */
#define RICOH619_SET_USB_CHARGE_CURRENT	  500	/* mA */
...
#define RICOH619_CHARGE_COMPLETION_CURRENT	200	/* mA Range 50~200
							 * (Step 50) */
#define RICOH619_FULL_CHARGING_VOLTAGE		4350	/* mv can set 4050,
							 * 4100, 4150, 4200,
							 * 4350(default 4100) */
#define	RICOH619_RE_CHARGING_VOLTAGE		4100	/* mv can set 3850,*/
/* FG setting */
#define RICOH619_REL1_SEL_VALUE		64	/* mv Range 0~240
							 * (Step 16) */
#define RICOH619_REL2_SEL_VALUE		0	/* mv Range 0~240
							 * (Step 16) */
unsigned int	RICOH619_TARGET_VBAT;
enum int_type {
	SYS_INT  = 0x01,
	DCDC_INT = 0x02,
	ADC_INT  = 0x08,
...
#define RICOH619_ENTER_LOW_VOL			60
...
struct ricoh619_battery_info *battreydev=NULL;
struct device*ricohdev=NULL;
int g_full_flag;
int charger_irq;
...
static void ricoh619_usb_current_set(struct ricoh619_battery_info *info );
extern int dwc_otg_check_dpdm(void);
static int  ricoh619_set_USB_charger_current(struct ricoh619_battery_info *info, int  status);
...
/*---------------------------*/
struct wake_lock ricoh619_wake_lock;
static unsigned char WakeLockFlag=0;
static void Ricoh619_ac_lock(void)
{
	unsigned long		flags;
	local_irq_save(flags);
	wake_lock(&ricoh619_wake_lock);
	local_irq_restore(flags);

}
static void Ricoh619_ac_unlock()
{
	unsigned long		flags;
	local_irq_save(flags);
	wake_unlock(&ricoh619_wake_lock);
	local_irq_restore(flags);
}
struct task_struct *detect_task;
#define RICOH619_CHARGER_DETECT_THREAD 1
int g_charger_mode = -1;
static int g_resume_mode = -1;

#if RICOH619_CHARGER_DETECT_THREAD
static int ricoh619_charger_detect_thread()
{
/* 
 * 0: No charging , 1: USB, 2: DC, 3: unknow
 */
	int status = -1;
	...
	do
	{
		...
		if (status == 0)
		{
			...
			info ->ac_online = 0;
			info ->usb_online = 0;
			ricoh619_clr_bits(info->dev->parent,RICOH619_PWR_REP_CNT,(1 << 0)); //clr restart when power
			power_supply_changed(&info ->ac);
			power_supply_changed(&info ->usb);
			power_supply_changed(&info ->bq24155);
		}else
		{

			ricoh619_set_bits(info->dev->parent,REGISET2_REG,(1 << 7));  //set usb limit current  when SDP or other mode
			...
			if(2 == status)
			{
				...
				info ->ac_online = 1;
				info ->usb_online = 0;
				ricoh619_set_USB_charger_current(info, status);

			}else if(1 == status)
			{	/*
				if (0 == get_gadget_connect_flag()){ 
					printk("half usb Insert\n");
					info ->ac_online = 0;
					info ->usb_online = 0;
					...
					info ->ac_online = 0;
					info ->usb_online = 1;
					ricoh619_set_USB_charger_current(info, 0);

				}

			}
			ricoh619_set_bits(info->dev->parent,RICOH619_PWR_REP_CNT,(1 << 0)); //set restart when power
			power_supply_changed(&info ->ac);
			power_supply_changed(&info ->usb);
			power_supply_changed(&info ->bq24155);
		}
...
#endif
extern uint8_t Half_Cable_Flage;
void ricoh619_charger_status(int status)
{
	#if RICOH619_CHARGER_DETECT_THREAD
	if (status > -1) 
	{
		int pc_or_charger = -1;
		/*
		if(Half_Cable_Flage == 1)
		{
			Half_Cable_Flage=0;
			wake_up_process(detect_task);
		}else
		*/
		{
			if(suspend_flag == 1)
			{
				...
				if (g_charger_mode != pc_or_charger) 
				{
					g_charger_mode = pc_or_charger;
					wake_up_process(detect_task);
				}
			}
			if(DC_detect==0)
...
EXPORT_SYMBOL(ricoh619_charger_status);

static int BatteryTableFlage=0;
static int Battery_Type(void)
{
	...
	bat_adc=adc_sync_read(client);
	bat_adc=bat_adc*1800/1024;//mv
	printk("bat_adc:%d\n",bat_adc);
	if(bat_adc>1400)//19w 4.2v --1.64v
	{
		BatteryTableFlage=0;
		return 0;
	}
	...
	{
		if(bat_adc<350)//15w 4.35v --0.16v
		{
			BatteryTableFlage=1;
			return 1;
		}
		else			//samsung 19w 4.2v
		{
			BatteryTableFlage=2;
			return 2;
		}
		//printk("choose Battery_Type fail\n");
	}
...
{
	if (BatteryTableFlage == 1)
	{
		RICOH619_TARGET_VBAT = 3450;
	}
	else
	{
		if ((BatteryTableFlage == 2) || (BatteryTableFlage == 0))
		{
			RICOH619_TARGET_VBAT = 3400;
		}
	}
	return BatteryTableFlage;
}
				 ...
				 int *cc_cap, bool *is_charging, bool cc_rst)
{
	...
	*cc_cap = cc_sum*25/9/fa_cap;		/* unit is 0.01% */
	...
	printk("PMU: cc_sum = %d: cc_cap_res= %d: \n", cc_sum, cc_cap_res);
...
static int get_target_use_cap(struct ricoh619_battery_info *info)
{
	...
	if (RE_CAP_now < RE_CAP_GO_DOWN) {
		...
		info->soca->hurry_up_flg = 1;
	} else if (info->soca->Vbat_ave < RICOH619_TARGET_VBAT*1000) {
		info->soca->hurry_up_flg = 1;
	} else {
...
static void ricoh619_displayed_work(struct work_struct *work)
{
		...
		if (info->soca->Ibat_ave >= -20) { /* charging */
			if (0 == info->soca->jt_limit) {
				if (g_full_flag == 1) {
					...
					printk("PMU : debug : %s : fc_delta is %d, fc_cap is %d",__func__,fc_delta, info->soca->fc_cap);
					printk("PMU : debug : %s : g_full_flag %d",__func__,g_full_flag);
					if (g_full_flag == 1){
						info->soca->displayed_soc = 100*100;
					}
				} else {
					if (calc_ocv(info) < (get_OCV_voltage(info, 8))) { /* fail safe*/
						...
						{
							info->soca->displayed_soc += (13 - info->soca->cc_delta);
						}
	...
	if (RICOH619_SOCA_DISP == info->soca->status) {
		...
		if(g_full_flag == 1){	/* for issue 1 solution start*/
			info->soca->status = RICOH619_SOCA_FULL;
			err = ricoh619_write(info->dev->parent, FG_CTRL_REG, 0x51);
			if (err < 0)
				dev_err(info->dev, "Error in writing the control register\n");
			info->soca->ready_fg = 0;
			goto end_flow;
		...
		ret = measure_Ibatt_FG(info, &Ibat);
		ocv = Vbat - Ibat * info->soca->Rbat;
		printk("Check ZERO state : Vbat %d, Ibat %d, ocv %d Rbat %d %d\n",Vbat,Ibat,ocv,info->soca->Rbat);
		info->soca->soc = calc_capacity(info) * 100;
		printk("Check ZERO state : first_pwon %d, RSOC is %d OCV is %d\n",info->first_pwon,info->soca->soc/100,calc_ocv(info));
		printk("Check ZERO state : g_fg_on_mode %d, get ocv voltage %d\n",g_fg_on_mode,get_OCV_voltage(info, 0));
		///////////////////////////
		if (info->first_pwon) {
			info->soca->soc = calc_capacity(info) * 100;
			...
			if ((info->soca->soc == 0) && (calc_ocv(info)
				...
				if (Ibat < 0) {
					...
					|| (Vbat < RICOH619_TARGET_VBAT * 1000) ) {
						info->soca->target_use_cap = 0;
						info->soca->status = RICOH619_SOCA_LOW_VOL;
					} else {
...
end_flow:
	...
	ricoh619_read(info->dev->parent, 0xbd, &val);
	printk("PMU:STATUS= %d: IBAT= %d: VSYS= %d: VBAT= %d: DSOC= %d: RSOC= %d: 0xBD:%x mode:%d %d\n",
	       info->soca->status, info->soca->Ibat_ave, info->soca->Vsys_ave, info->soca->Vbat_ave,
	info->soca->displayed_soc, info->soca->soc,val,g_charger_mode,get_gadget_connect_flag());
...
static void ClearWatchDog_work(struct work_struct *work)
{
	struct ricoh619_battery_info *info = battreydev;

	//printk("Clear WatchDog Flage**********\n");
	
	ricoh619_set_bits(info->dev->parent, 0x0b, 0x03);
	queue_delayed_work(info->monitor_wqueue, &info->watchdog_work,15 * HZ);
...
*/
{
	...
	*is_jeita_updated = false;
	...
	if (info->soca->ready_fg) {
		...
		printk("%s  cannot update by resetting FG \n", __func__);
		goto out;
	}
	...
	printk("PMU:Temperature: %d, vfchg: %d, SW status: %d, chg_status: %d \n",
		  temp, vfchg, info->soca->status, info->soca->chg_status);

	if (temp <= 0 || 55 <= temp) {
		/* 1st and 5th temperature ranges (~0, 55~) */
		//printk("PMU: Temp(%d) is out of 0-55\n", temp);
		printk("Find: Stop charge 1\n");
		...
		*is_jeita_updated = true;
		...
		if (vfchg != info->jt_vfchg_h) {
			//printk("PMU: 0<Temp<12, update to vfchg=%d \n", 
			//						__func__, info->jt_vfchg_h);
			printk("Find: Stop charge 2\n");
			err = ricoh619_clr_bits(info->dev->parent, CHGCTL1_REG, 0x03);
			if (err < 0) {
				dev_err(info->dev, "Error in writing the control register\n");
				goto out;
		...
		if (vfchg != info->jt_vfchg_h) {
			//printk("PMU: 12<Temp<50, update to vfchg=%d \n", info->jt_vfchg_h);
			printk("Find: Stop charge 3\n");
			err = ricoh619_clr_bits(info->dev->parent, CHGCTL1_REG, 0x03);
			if (err < 0) {
				dev_err(info->dev, "Error in writing the control register\n");
				goto out;
			...
			//printk("PMU: 12<Temp<50, already set vfchg==%d\n", 
			//		info->jt_vfchg_h);
		...
		if (vfchg != info->jt_vfchg_l) {
			//printk("PMU: 50<Temp<55, update to vfchg=%d \n",info->jt_vfchg_l);
			printk("Find: Stop charge 4\n");
			err = ricoh619_clr_bits(info->dev->parent, CHGCTL1_REG, 0x03);
			if (err < 0) {
				dev_err(info->dev, "Error in writing the control register\n");
				goto out;
			...
			*is_jeita_updated = true;
			...
			printk("JEITA: 50<Temp<55, already set vfchg=%d\n", 
					info->jt_vfchg_l);
	...
	get_power_supply_status(info);
	//printk("PMU: chg_status: %d, is_jeita_updated: %d\n",info->soca->chg_status, *is_jeita_updated);
...
static void ricoh619_jeita_work(struct work_struct *work)
{
	...
	if (0 == ret) {
		...
		printk(KERN_INFO "PMU:  Call check_jeita_status() in jeita_work, err:%d \n", ret);
		queue_delayed_work(info->monitor_wqueue, &info->jeita_work,
					 RICOH619_FG_RESET_TIME * HZ);
	}
...
static int ricoh619_set_OCV_table(struct ricoh619_battery_info *info)
{
	...
	//for debug
	printk("PMU : target_vsys is %d target_ibat is %d\n",info->soca->target_vsys,info->soca->target_ibat);
	...
	for (i = 0; i < 11; i++){
		...
		//printk("PMU : %s : no %d OCV_set_table %x\n", __func__, i, OCV_set_table[i]);
	...
	//printk("PMU : def OCV 100 %x OCV 0 %x \n", OCV_100_def, OCV_0_def);
	//printk("PMU : OCV 100 %x OCV 0 %x \n", OCV_set_table[10], OCV_set_table[0]);
	//printk("PMU : diff OCV 100 %x OCV 0 %x \n",  OCV_100_diff, OCV_0_diff);
...
static int  ricoh619_set_ADP_charger_current(struct ricoh619_battery_info *info)
{
	int err = 0;
	uint8_t val;
	uint8_t val2;
	...
	if (RICOH619_MAX_ADP_CHARGE_CURRENT != 0) {
		/* Change ADP Current to 2.5A. */
		err = ricoh619_write(info->dev->parent, REGISET1_REG,
						 (RICOH619_MAX_ADP_CHARGE_CURRENT-1)/100);
		if (err < 0) {
	...
	if (RICOH619_SET_ADP_CHARGE_CURRENT != 0) {
		val = (RICOH619_SET_ADP_CHARGE_CURRENT-1)/100;
	} else {
		err = ricoh619_read(info->dev->parent, CHGISET_REG, &val);
		if (err < 0) {
			dev_err(info->dev,
...
//xxxxxxxxxxxxxxxxxxxxxxxxxxxx
	...
	if(status == 0){
		if (RICOH619_MAX_USB_CHARGE_CURRENT != 0) {
			...
			val2 &= 0xE0;
			val = (RICOH619_MAX_USB_CHARGE_CURRENT-1)/100;
			val = val + val2;
		...
		if (RICOH619_SET_USB_CHARGE_CURRENT != 0) {
			val = (RICOH619_SET_USB_CHARGE_CURRENT-1)/100;
		} else {
			err = ricoh619_read(info->dev->parent, CHGISET_REG, &val);
			if (err < 0) {
				dev_err(info->dev,
		...
		if (RICOH619_MAX_ADP_CHARGE_CURRENT != 0) {
			...
			val2 &= 0xE0;
			val = (RICOH619_MAX_ADP_CHARGE_CURRENT-1)/100;
			val = val + val2;
			err = ricoh619_write(info->dev->parent, REGISET2_REG, val);
			if (err < 0) {
				dev_err(info->dev,
...
static int ricoh619_init_charger(struct ricoh619_battery_info *info)
{
	...
	//printk("PMU : %s : before REGISET1_REG (0x%x) is 0x%x info->ch_ilim_adp is 0x%x\n",__func__,REGISET1_REG,val,info->ch_ilim_adp);
	...
	//debug messeage
	err = ricoh619_read(info->dev->parent, REGISET1_REG,&val);
	//printk("PMU : %s : after REGISET1_REG (0x%x) is 0x%x info->ch_ilim_adp is 0x%x\n",__func__,REGISET1_REG,val,info->ch_ilim_adp);
	
		//debug messeage
	err = ricoh619_read(info->dev->parent, REGISET2_REG,&val);
	//printk("PMU : %s : before REGISET2_REG (0x%x) is 0x%x info->ch_ilim_usb is 0x%x\n",__func__,REGISET2_REG,val,info->ch_ilim_usb);
	...
	if (err < 0) {
		...
		//debug messeage
	err = ricoh619_read(info->dev->parent, REGISET2_REG,&val);
	//printk("PMU : %s : after REGISET2_REG (0x%x) is 0x%x info->ch_ilim_usb is 0x%x\n",__func__,REGISET2_REG,val,info->ch_ilim_usb);

	/* CHGISET_REG(0xB8) setting */
		//debug messeage
	err = ricoh619_read(info->dev->parent, CHGISET_REG,&val);
	//printk("PMU : %s : before CHGISET_REG (0x%x) is 0x%x info->ch_ichg is 0x%x info->ch_icchg is 0x%x\n",__func__,CHGISET_REG,val,info->ch_ichg,info->ch_icchg);
	...
	if (err < 0) {
		...
		//debug messeage
	err = ricoh619_read(info->dev->parent, BATSET2_REG,&val);
	//printk("PMU : %s : after BATSET2_REG (0x%x) is 0x%x info->ch_vrchg is 0x%x info->ch_vfchg is 0x%x  \n",__func__,BATSET2_REG,val,info->ch_vrchg,info->ch_vfchg);
...
{
	struct ricoh619 *ricoh619 = g_ricoh619;
	int status,status_619;
	msleep(1000);
	status =  dwc_vbus_status();
//	status_619 = get_power_supply_status(info);
	if ((status == 0)/*&&(status_619 != POWER_SUPPLY_STATUS_CHARGING )*/){
	//	printk("-------------dwc_vbus_status---------status_619=%d-----\n  ", status_619);
		info ->ac_online = 0;
		info ->usb_online = 0;
		power_supply_changed(&info ->ac);
		power_supply_changed(&info ->usb);
	...
	ricoh619_set_bits(ricoh619->dev,REGISET2_REG,(1 << 7));  //set usb limit current  when SDP or other mode
	//printk("dwc_vbus_status === %d\n", dwc_vbus_status());
	if(2 == status){
	//	printk("charge......................AC\n");
		info ->ac_online = 1;
		info ->usb_online = 0;
		//ricoh619_write(ricoh619->dev,REGISET2_REG,0x0e);  //set usb limit current  2A
		//ricoh619_write(ricoh619->dev,CHGISET_REG,0xcc);  //set charge current  2A
		ricoh619_set_USB_charger_current(info, status);
...
static irqreturn_t charger_usb_isr(int irq, void *battery_info)
{
	...
	ricoh619_read(info->dev->parent, 0xBD, &sts);
	if((sts&0xc0) == 0)
	{
		info ->ac_online = 0;
		info ->usb_online = 0;
...
static int get_battery_temp_2(struct ricoh619_battery_info *info)
{
	...
	if (0 != temp_gain)
		...
		printk("PMU %s Skip to compensate temperature\n", __func__);
		goto out;
	}
	...
	if (temp < -120000) {
		...
		printk("PMU %s low temperature %d\n", __func__, new_temp/10);  
	} else if (temp > 520000) {
		/* High Temperature */
		if (0 != (2500 - Vthm)) {
		...
		printk("PMU %s high temperature %d\n", __func__, new_temp/10);  
	} else {
		/* the unit of new_temp is 0.1 degree */
		new_temp = temp / 1000;
	}
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CURRENT_AVG:
		...
		//printk("average current xxxxxxxxxxxxxx %d \n", data);
		break;
	default:
		mutex_unlock(&info->lock);
		return -ENODEV;
...
static int ricoh619_bq_get_prop(struct power_supply *psy,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
			if (psy->type == POWER_SUPPLY_TYPE_USB){
				val->intval = info ->usb_online; ///(status & 0x80 ? 1 : 0);
			//	printk("POWER_SUPPLY_TYPE_USB xxxxx=== %d\n", info->usb_online);
...
static int ricoh619_ac_get_prop(struct power_supply *psy,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
	struct ricoh619_battery_info *info = dev_get_drvdata(psy->dev->parent);
	...
	case POWER_SUPPLY_PROP_ONLINE:
			if (psy->type == POWER_SUPPLY_TYPE_MAINS){
				val->intval = info ->ac_online; //(status & 0x40 ? 1 : 0);
				//printk("POWER_SUPPLY_TYPE_MAINS xxxxxxx=== %d\n", info->ac_online);
			}
			break;
	default:
		ret = -EINVAL;
...
static int ricoh619_usb_get_prop(struct power_supply *psy,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
...
struct power_supply	bq24155 = {
		...
		.get_property = ricoh619_bq_get_prop,
};
struct power_supply	powerac = {
		...
		.properties = ricoh619_power_props,
		.num_properties = ARRAY_SIZE(ricoh619_power_props),
		.get_property = ricoh619_ac_get_prop,
};
...
struct power_supply	powerusb = {
		...
		.properties = ricoh619_power_props,
		.num_properties = ARRAY_SIZE(ricoh619_power_props),
		.get_property = ricoh619_usb_get_prop,
};

static void ricoh619_poweron_charge_check(struct ricoh619_battery_info *info)
{
	struct ricoh619 *ricoh619 = g_ricoh619;
	...
	ricoh619_set_bits(ricoh619->dev,REGISET2_REG,(1 << 7));  //set usb limit current  when SDP or other mode
	charge_status  = dwc_otg_check_dpdm();
	if(charge_status == 0){
		info ->ac_online = 0;
		info ->usb_online = 0;	
		ricoh619_clr_bits(info->dev->parent,RICOH619_PWR_REP_CNT,(1 << 0)); //clr restart when power
	}else if (2 == charge_status){
		info ->ac_online = 1;
		info ->usb_online = 0;	
		ricoh619_set_USB_charger_current(info, charge_status);
		ricoh619_set_bits(info->dev->parent,RICOH619_PWR_REP_CNT,(1 << 0)); //set restart when power
		//ricoh619_write(ricoh619->dev,REGISET2_REG,0x0e);  //set usb limit current  2A
		//ricoh619_write(ricoh619->dev,CHGISET_REG,0xcc);  //set charge current  2A

	}else if(1 == charge_status ){
		info ->ac_online = 0;
		info ->usb_online = 1;	
		ricoh619_set_USB_charger_current(info, 0);
		ricoh619_set_bits(info->dev->parent,RICOH619_PWR_REP_CNT,(1 << 0)); //set restart when power
	...
	power_supply_changed(&info ->bq24155);
	return;
//	printk("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n");
				      ...
				      size_t count)
{
	...
	info=battreydev;
	 ...
	 if (strncmp(buf, "off", 3) == 0) 
 	{
		...
		ret = ricoh619_set_bits(info->dev->parent, CHGCTL1_REG, 0x08);
		ret = ricoh619_clr_bits(info->dev->parent, CHGCTL1_REG, 0x03);
		if (ret < 0) {
			dev_err(info->dev, "Error in writing the control register\n");
			return ret;
		}
 	...
 	}else if (strncmp(buf, "dedicated", 9) == 0)
	{
		...
		ret = ricoh619_clr_bits(info->dev->parent,CHGCTL1_REG, 0x08);
		ret = ricoh619_set_bits(info->dev->parent, CHGCTL1_REG, 0x03);
		if (ret < 0) {
			dev_err(info->dev, "Error in writing the control register\n");
			return ret;
		}
		info ->ac_online = 1;
		info ->usb_online = 0;
		ricoh619_set_USB_charger_current(info, 2);
		suspend_flag=1;
		/**/
		if(WakeLockFlag==0){
			Ricoh619_ac_lock();
				       ...
				       size_t count)//ward_du
{
	...
	if (strncmp(buf, "close", 5) == 0) 
	{
		...
		ricoh619_clr_bits(info->dev->parent,CHGCTL1_REG, 0x08);
		ricoh619_set_bits(info->dev->parent, CHGCTL1_REG, 0x03);
		status =  2;
		if (status == 0)
		{
			info ->ac_online = 0;
...
static int bq2415x_sysfs_init(struct ricoh619_battery_info *bq)
{
	//DBG("In the Funtion %s.\n", __FUNCTION__);

	return sysfs_create_group(&bq->bq24155.dev->kobj,
			&bq2415x_sysfs_attr_group);
}

static void bq2415x_sysfs_exit(struct ricoh619_battery_info *bq)
{
	//DBG("In the Funtion %s.\n", __FUNCTION__);

	sysfs_remove_group(&bq->bq24155.dev->kobj, &bq2415x_sysfs_attr_group);
}
//--------------------------------------------------------------------
static __devinit int ricoh619_battery_probe(struct platform_device *pdev)
{
	...
	printk("PMU_Driver_Version:2013.12.05 \n");
	...
	//ricohdev = kzalloc(sizeof(struct device), GFP_DMA | GFP_KERNEL);	
	ricohdev = &pdev->dev;
	info->dev = &pdev->dev;
	...
	info->jt_en = pdata->jt_en;
        ...
        info->jt_ichg_l = pdata->jt_ichg_l;
			...
			{
	info->ch_vfchg = pdata->ch_vfchg2;
	info->ch_vrchg = pdata->ch_vrchg2;
	info->ch_vbatovset = pdata->ch_vbatovset2;
	info->ch_ichg = pdata->ch_ichg2;
	info->ch_ilim_adp = pdata->ch_ilim_adp2;
	info->ch_ilim_usb = pdata->ch_ilim_usb2;
	info->ch_icchg = pdata->ch_icchg2;
	info->fg_target_vsys = pdata->fg_target_vsys2;
	info->fg_target_ibat = pdata->fg_target_ibat2;
	info->jt_en = pdata->jt_en2;
        info->jt_hw_sw = pdata->jt_hw_sw2;
        info->jt_temp_h = pdata->jt_temp_h2;
        info->jt_temp_l = pdata->jt_temp_l2;
        info->jt_vfchg_h = pdata->jt_vfchg_h2;
        info->jt_vfchg_l = pdata->jt_vfchg_l2;
        info->jt_ichg_h = pdata->jt_ichg_h2;
        info->jt_ichg_l = pdata->jt_ichg_l2;
			}
		/*
	printk("%s pdata->type[type_n].ch_vfchg = 0x%x\n", __func__, pdata->type[type_n].ch_vfchg);
	printk("%s pdata->type[type_n].ch_vrchg = 0x%x\n", __func__, pdata->type[type_n].ch_vrchg);
	printk("%s pdata->type[type_n].ch_vbatovset =0x%x\n", __func__, pdata->type[type_n].ch_vbatovset);
	printk("%s pdata->type[type_n].ch_ichg = 0x%x\n", __func__, pdata->type[type_n].ch_ichg);
	printk("%s pdata->type[type_n].ch_ilim_adp =0x%x \n", __func__, pdata->type[type_n].ch_ilim_adp);
	printk("%s pdata->type[type_n].ch_ilim_usb = 0x%x\n", __func__, pdata->type[type_n].ch_ilim_usb);
	printk("%s pdata->type[type_n].ch_icchg = 0x%x\n", __func__, pdata->type[type_n].ch_icchg);
	printk("%s pdata->type[type_n].fg_target_vsys = 0x%x\n", __func__, pdata->type[type_n].fg_target_vsys);
	printk("%s pdata->type[type_n].fg_target_ibat = 0x%x\n", __func__, pdata->type[type_n].fg_target_ibat);
	printk("%s pdata->type[type_n].jt_en = 0x%x\n", __func__, pdata->type[type_n].jt_en;
        printk("%s pdata->type[type_n].jt_hw_sw = 0x%x\n", __func__, pdata->type[type_n].jt_hw_sw;
        printk("%s pdata->type[type_n].jt_temp_h = 0x%x\n", __func__, pdata->type[type_n].jt_temp_h;
        printk("%s pdata->type[type_n].jt_temp_l = 0x%x\n", __func__, pdata->type[type_n].jt_temp_l;
        printk("%s pdata->type[type_n].jt_vfchg_h = 0x%x\n", __func__, pdata->type[type_n].jt_vfchg_h;
        printk("%s pdata->type[type_n].jt_vfchg_l = 0x%x\n", __func__, pdata->type[type_n].jt_vfchg_l;
        printk("%s pdata->type[type_n].jt_ichg_h = 0x%x\n", __func__, pdata->type[type_n].jt_ichg_h;
        printk("%s pdata->type[type_n].jt_ichg_l = 0x%x\n", __func__, pdata->type[type_n].jt_ichg_l;	
	*/
...
/*	info->battery.external_power_changed
	...
	//battreydev = kzalloc(sizeof(struct ricoh619_battery_info), GFP_KERNEL);
	//if (!battreydev)
	//	return -ENOMEM;
	battreydev=info;
	ricoh619_clr_bits(info->dev->parent,RICOH619_PWR_REP_CNT,(1 << 0)); //clr restart when power
	wake_lock_init(&ricoh619_wake_lock, WAKE_LOCK_SUSPEND,"ricoh619_wakelock");
	...
	//--------------
	info ->bq24155 = bq24155;
	ret = power_supply_register(&pdev->dev, &info->bq24155);
	ret = bq2415x_sysfs_init(info);
	//--------------
	info->monitor_wqueue
	...
	INIT_DELAYED_WORK_DEFERRABLE(&info->watchdog_work,ClearWatchDog_work);
...
//					RICOH619_MONITOR_START_TIME*HZ);
	...
	ricoh619_poweron_charge_check(info);
	#if RICOH619_CHARGER_DETECT_THREAD
	detect_task = kthread_run(ricoh619_charger_detect_thread, NULL, "ricoh619_detect");
	if (!IS_ERR(detect_task)) 
	{
		struct sched_param param = { .sched_priority = MAX_RT_PRIO - 1 };
		sched_setscheduler_nocheck(detect_task, SCHED_RR, &param);
	}
	#endif
	//Enable WatchDog 128s ,When soc suspend ,stop the countdown
	ricoh619_write(info->dev->parent, 0x0B, 0x07);
	ricoh619_read(info->dev->parent,0x12, &reg);	
	ricoh619_write(info->dev->parent,0x12, (reg|0x40));
	queue_delayed_work(info->monitor_wqueue, &info->watchdog_work,15 * HZ);
	
	ricoh619_read(info->dev->parent, 0x0a, &reg);
	printk("PMU: Last shut down reason: %x\n",reg);
	ricoh619_read(info->dev->parent, 0x09, &reg);
	printk("PMU: Last restart reason: %x\n",reg);
	return 0;
...
static void ricoh619_shutdown(struct platform_device *pdev)
{
	struct ricoh619_battery_info *info = battreydev;

	//Close WatchDog 
	ricoh619_clr_bits(info->dev->parent, 0x0B, 0x04);
	ricoh619_clr_bits(info->dev->parent, 0x12, 0x40);
	ricoh619_clr_bits(info->dev->parent, 0x0B, 0x04);
	
	cancel_delayed_work(&info->watchdog_work);
...
#endif
	flush_delayed_work(&info->watchdog_work);
	flush_delayed_work(&info->monitor_work);
	flush_delayed_work(&info->displayed_work);
	flush_delayed_work(&info->charge_stable_work);
	flush_delayed_work(&info->charge_monitor_work);
...
//	enable_irq(charger_irq + RICOH619_IRQ_VSYSLIR);
	...
	if(suspend_flag==0)
	{
		printk("Find: Open charge 7\n");
		ricoh619_clr_bits(info->dev->parent, CHGCTL1_REG, 0x03);
		ricoh619_set_bits(info->dev->parent, CHGCTL1_REG, 0x08);
	}*/
...
static struct platform_driver ricoh619_battery_driver = {
	...
	.shutdown = ricoh619_shutdown,
};
//----------------------realrsoc
static ssize_t readBatteryReg(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	static uint8_t realrsoc=0;
	int ret = 0;

	if (battreydev->soca->ready_fg) 
	{
		/* get remaining battery capacity from fuel gauge */
		ret = ricoh619_read(battreydev->dev->parent, 0xe1, &realrsoc);
		if (ret < 0) {
			dev_err(battreydev->dev->parent, "Error in reading the control register\n");
			return ret;
		}
	}
...
//---Capacity
static ssize_t readBatteryBatteryCapacity(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	int err;
	uint8_t fa_cap_reg[2];
	uint16_t fa_cap;
	...
	/* Read FA_CAP */
	err = ricoh619_bulk_reads(battreydev->dev->parent,
				 FA_CAP_H_REG, 2, fa_cap_reg);
...
//---Current
static ssize_t readBatteryBatteryCurrent(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	int ret = 0;
	uint Current;

	ret =  get_check_fuel_gauge_reg(battreydev, CC_AVERAGE1_REG,
						 CC_AVERAGE0_REG, 0x3fff);
	if (ret < 0) {
		dev_err(battreydev->dev, "Error in reading the fuel gauge control register\n");
		return ret;
	}

	Current = (ret > 0x1fff) ? (0x4000 - ret) : ret;
...
//---Temp
static ssize_t readBatteryBatteryTemp(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	static int ret = 0;
	int sign_bit;
	...
	if (battreydev->soca->ready_fg) 
	{
		ret = get_check_fuel_gauge_reg(battreydev, TEMP_1_REG, TEMP_2_REG, 0x0fff);
		if (ret < 0) {
			dev_err(battreydev->dev, "Error in reading the fuel gauge control register\n");
			return ret;
		}
...
//---ID
static ssize_t readBatteryBatteryID(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	if (BatteryTableFlage == 0)
		return snprintf(page, PAGE_SIZE,"4.20v LG\n");
	else	/*negative value part */
	{
		if (BatteryTableFlage == 1)
			return snprintf(page, PAGE_SIZE,"4.35v \n");	
		else
		{
			if (BatteryTableFlage == 2)
				return snprintf(page, PAGE_SIZE,"4.20v SDI\n");	
		}
	}
}
//---Cell ID
static ssize_t readBatteryCellID(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	return snprintf(page, PAGE_SIZE,"%d",battreydev->soca->status);	
}

//---target_use_cap
static ssize_t readBatteryTarget_Use_Cap(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	return snprintf(page, PAGE_SIZE,"%d",battreydev->soca->target_use_cap);	
}

//---calc_ocv
static ssize_t readBatteryCalc_OCV(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	int ocv;
	ocv = calc_ocv(battreydev);
	return snprintf(page, PAGE_SIZE,"%d",ocv);	
}

//---g_full_flage
static ssize_t readBatteryG_Full_Flage(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	return snprintf(page, PAGE_SIZE,"%d",g_full_flag);	
}

//---ReCapacity
static ssize_t readBatteryBatteryReCapacity(char* page,char** start, off_t off, int count, int* eof, void* data)
{
	int err;
	uint8_t re_cap_reg[2];
	uint16_t re_cap;

	/* Read FA_CAP */
	err = ricoh619_bulk_reads(battreydev->dev->parent,
				 RE_CAP_H_REG, 2, re_cap_reg);

	/* fa_cap = *(uint16_t*)fa_cap_reg & 0x7fff; */
	re_cap = (re_cap_reg[0] << 8 | re_cap_reg[1]) & 0x7fff;
...
{
	struct proc_dir_entry *BatteryReg;
	struct proc_dir_entry *BatteryCapacity;	
	struct proc_dir_entry *BatteryCurrent;
	struct proc_dir_entry *BatteryTemp;		
	struct proc_dir_entry *BatteryID;
	struct proc_dir_entry *BatteryCellID;
	struct proc_dir_entry *BatteryTarget_Use_Cap;
	struct proc_dir_entry *BatteryCalc_OCV;
	struct proc_dir_entry *BatteryG_Full_Flage;
	struct proc_dir_entry *BatteryReCapacity;	

	BatteryReg = create_proc_entry("BatteryReg",0,NULL);
	if(BatteryReg)
		BatteryReg->read_proc = readBatteryReg;

	BatteryCapacity = create_proc_entry("BatteryCapacity",0,NULL);
	if(BatteryCapacity)
		BatteryCapacity->read_proc = readBatteryBatteryCapacity;

	BatteryCurrent = create_proc_entry("BatteryCurrent",0,NULL);
	if(BatteryCurrent)
		BatteryCurrent->read_proc = readBatteryBatteryCurrent;

	BatteryTemp = create_proc_entry("BatteryTemp",0,NULL);
	if(BatteryTemp)
		BatteryTemp->read_proc = readBatteryBatteryTemp;					
	BatteryID = create_proc_entry("BatteryID",0,NULL);
	if(BatteryID)
		BatteryID->read_proc = readBatteryBatteryID;		

	BatteryCellID = create_proc_entry("BatteryCellID",0,NULL);
	if(BatteryCellID)
		BatteryCellID->read_proc = readBatteryCellID;

		BatteryTarget_Use_Cap = create_proc_entry("BatteryTarget_Use_Cap",0,NULL);
	if(BatteryTarget_Use_Cap)
		BatteryTarget_Use_Cap->read_proc = readBatteryTarget_Use_Cap;

		BatteryCalc_OCV = create_proc_entry("BatteryCalc_OCV",0,NULL);
	if(BatteryCalc_OCV)
		BatteryCalc_OCV->read_proc = readBatteryCalc_OCV;

		BatteryG_Full_Flage = create_proc_entry("BatteryG_Full_Flage",0,NULL);
	if(BatteryG_Full_Flage)
		BatteryG_Full_Flage->read_proc = readBatteryG_Full_Flage;
	
		BatteryReCapacity = create_proc_entry("BatteryReCapacity",0,NULL);
	if(BatteryReCapacity)
		BatteryReCapacity->read_proc = readBatteryBatteryReCapacity;
}

static void freg_remove_proc(void)
{
	remove_proc_entry("BatteryReg",NULL);
	remove_proc_entry("BatteryCapacity",NULL);
	remove_proc_entry("BatteryCurrent",NULL);
	remove_proc_entry("BatteryTemp",NULL);
	remove_proc_entry("BatteryID",NULL);
	remove_proc_entry("BatteryCellID",NULL);	
	remove_proc_entry("BatteryTarget_Use_Cap",NULL);
	remove_proc_entry("BatteryCalc_OCV",NULL);
	remove_proc_entry("BatteryG_Full_Flage",NULL);
	remove_proc_entry("BatteryReCapacity",NULL);
}
//----------------------
