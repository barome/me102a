...
static int store_sku_info(void)
{
	u8 tmp_buffer[512] = {0};

//	printk(KERN_ERR "kernel version: %s\n", ASUS_BUILD_KERNEL_VERSION);
		
	if(rk_emmc_transfer(tmp_buffer, 8064+4, 512, 0) != 0)
	{
		printk(KERN_ERR "failed to read private space\n");
		return -1;
	}
	...
	if(tmp_buffer[4] == 0)
	{
		printk("write SKU info\n");
		if(strncmp(ASUS_BUILD_KERNEL_VERSION, "WW", 2) == 0)
			tmp_buffer[4] = 2;
		else if(strncmp(ASUS_BUILD_KERNEL_VERSION, "CN", 2) == 0)
			tmp_buffer[4] = 3;
		else if(strncmp(ASUS_BUILD_KERNEL_VERSION, "TW", 2) == 0)
			tmp_buffer[4] = 4;
		else if(strncmp(ASUS_BUILD_KERNEL_VERSION, "JP", 2) == 0)
			tmp_buffer[4] = 5;
		else
		{
			printk(KERN_ERR "bad SKU! please check your kernel image\n");
			return -1;
		}
		
		if(rk_emmc_transfer(tmp_buffer, 8064+4, 512, 1) != 0)
		{
			printk(KERN_ERR "failed to write data back to private space\n");
			return -1;
		}
	}
...
if (HOST_IS_EMMC(card->host)) {
	if(store_sku_info() != 0)
		printk(KERN_ERR "store sku info failed\n");
}
	return 0;
