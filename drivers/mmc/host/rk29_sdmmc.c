...
#else
#define RK29_MAX_SDIO_FREQ   48000000    //set max-sdio-frequency 48Mhz at the present time	//<ASUS-Aled_Zhang20130704>
#endif
...
struct workqueue_struct *workqueuein;
struct work_struct	power_inwork;
struct workqueue_struct *workqueueout;
struct work_struct	power_outwork;
...
int mmc0_io_switch(int mode)
{
	switch(mode){
		case GPIO_MODE:
			printk("GPIO MODE\n");
#if 1
			iomux_set_gpio_mode(mmc0_io.gpio_d0);
			iomux_set_gpio_mode(mmc0_io.gpio_d1);
			iomux_set_gpio_mode(mmc0_io.gpio_d2);
			iomux_set_gpio_mode(mmc0_io.gpio_d3);
			iomux_set_gpio_mode(mmc0_io.gpio_cmd);
			iomux_set_gpio_mode(mmc0_io.gpio_clkout);

			gpio_set_value(mmc0_io.gpio_d0, GPIO_LOW);
			gpio_set_value(mmc0_io.gpio_d1, GPIO_LOW);
			gpio_set_value(mmc0_io.gpio_d2, GPIO_LOW);
			gpio_set_value(mmc0_io.gpio_d3, GPIO_LOW);
			gpio_set_value(mmc0_io.gpio_cmd, GPIO_LOW);
			gpio_set_value(mmc0_io.gpio_clkout, GPIO_LOW);

			gpio_direction_input(mmc0_io.gpio_d0);
			gpio_direction_input(mmc0_io.gpio_d1);
			gpio_direction_input(mmc0_io.gpio_d2);
			gpio_direction_input(mmc0_io.gpio_d3);
			gpio_direction_input(mmc0_io.gpio_cmd);
			gpio_direction_input(mmc0_io.gpio_clkout);
#endif
			gpio_pull_updown(mmc0_io.gpio_d0, 0);
			gpio_pull_updown(mmc0_io.gpio_d1, 0);
			gpio_pull_updown(mmc0_io.gpio_d2, 0);
			gpio_pull_updown(mmc0_io.gpio_d3, 0);
			gpio_pull_updown(mmc0_io.gpio_cmd, 0);
			gpio_pull_updown(mmc0_io.gpio_clkout, 0);
			break;
		case MMC0_MODE:
			printk("MMC0 MODE\n");
			gpio_pull_updown(mmc0_io.gpio_d0, 1);
			gpio_pull_updown(mmc0_io.gpio_d1, 1);
			gpio_pull_updown(mmc0_io.gpio_d2, 1);
			gpio_pull_updown(mmc0_io.gpio_d3, 1);
			gpio_pull_updown(mmc0_io.gpio_cmd, 1);
			gpio_pull_updown(mmc0_io.gpio_clkout, 1);
#if 1
			gpio_direction_output(mmc0_io.gpio_d0, GPIO_HIGH);
			gpio_direction_output(mmc0_io.gpio_d1, GPIO_HIGH);
			gpio_direction_output(mmc0_io.gpio_d2, GPIO_HIGH);
			gpio_direction_output(mmc0_io.gpio_d3, GPIO_HIGH);
			gpio_direction_output(mmc0_io.gpio_cmd, GPIO_HIGH);
			gpio_direction_output(mmc0_io.gpio_clkout, GPIO_HIGH);
...
int mmc0_io_init(void)
{
	int ret = 0;

	mmc0_io.gpio_d0 = iomux_mode_to_gpio(MMC0_D0);	
	mmc0_io.gpio_d1 = iomux_mode_to_gpio(MMC0_D1);	
	mmc0_io.gpio_d2 = iomux_mode_to_gpio(MMC0_D2);	
	mmc0_io.gpio_d3 = iomux_mode_to_gpio(MMC0_D3);	
	mmc0_io.gpio_cmd = iomux_mode_to_gpio(MMC0_CMD);	
	mmc0_io.gpio_clkout = iomux_mode_to_gpio(MMC0_CLKOUT);	

	ret = gpio_request(mmc0_io.gpio_d0, "MMC0_D0");
	if(ret < 0)
		return ret;
	ret = gpio_request(mmc0_io.gpio_d1, "MMC0_D1");
	if(ret < 0)
		return ret;
	ret = gpio_request(mmc0_io.gpio_d2, "MMC0_D2");
	if(ret < 0)
		return ret;
	ret = gpio_request(mmc0_io.gpio_d3, "MMC0_D3");
	if(ret < 0)
		return ret;
	ret = gpio_request(mmc0_io.gpio_cmd, "MMC0_CMD");
	if(ret < 0)
		return ret;
	ret = gpio_request(mmc0_io.gpio_clkout, "MMC0_CLKOUT");
	if(ret < 0)
		return ret;

	mmc0_io_switch(GPIO_MODE);
...
out:	   
    rk29_sdmmc_enable_irq(host, true);
    if((ios->power_mode  == MMC_POWER_OFF) && (RK29_CTRL_SDMMC_ID == host->pdev->id))
	mmc0_io_switch(GPIO_MODE);
...
static void enable_sd_inpower(struct work_struct *work)
{
	struct regulator *ldo4;
	//printk("sd ----------insert\n");
	ldo4 = regulator_get(NULL, "ricoh_ldo4");	
	regulator_set_voltage(ldo4, 3300000, 3300000);
	regulator_enable(ldo4);
	regulator_put(ldo4);
}

static void enable_sd_outpower(struct work_struct *work)
{
	struct regulator *ldo4;
	
	//printk("------------------enable_sd_outpower\n");
	ldo4 = regulator_get(NULL, "ricoh_ldo4");	
	if (ldo4 == NULL || IS_ERR(ldo4))
	{
		printk("get ldo4 failed!\n");
		return;
	}
	while(regulator_is_enabled(ldo4)>0)	
...
static irqreturn_t rk29_sdmmc_interrupt(int irq, void *dev_id)
{
    ...
    if(pending & SDMMC_INT_CD) 
    {
    	...
    	{    	    
			...
			if(present==1)
				queue_work(workqueuein, &power_inwork);
			else
				queue_work(workqueueout, &power_outwork);
...
static int rk29_sdmmc_probe(struct platform_device *pdev)
{
	...
	*/
	...
	workqueuein = create_singlethread_workqueue("enablesdpowerin");
	INIT_WORK(&power_inwork, enable_sd_inpower);
	workqueueout = create_singlethread_workqueue("enablesdpowerout");
	INIT_WORK(&power_outwork, enable_sd_outpower);
   ...
   if(RK29_CTRL_SDMMC_ID == host->pdev->id)   
	...
	if(strcasecmp(host->dma_name,"sd_mmc")==0)
	{
		Smm0present = rk29_sdmmc_get_cd(host->mmc);
		if(Smm0present==1)
		{
			//struct regulator *ldo4;
			//printk("------------------sd init\n");
