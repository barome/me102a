			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	case GSENSOR_IOCTL_APP_SET_RATE:
		...
		break;
	case GSENSOR_IOCTL_APP_SET_OFFSET:
		if (copy_from_user(cachedData->offset, argp, sizeof(cachedData->offset)))
		{
			result = -EFAULT;
			goto error;
		}
		cachedData->calibrationFlag = GSENSOR_NEED_CALIBRATION;
		break;
	default:
		break;
	}
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	int result = 0;
	short flag;
	struct ECS_cached_data * cachedData = (struct ECS_cached_data *)sensor->sensor_data;
	...
	switch (cmd) {
		...
		case ECS_IOCTL_APP_SET_DELAY:
			if (copy_from_user(cachedData->delay, argp, AKM_DELAY_LENGTH)) {
				return -EFAULT;
			}
			break;
		default:
	...
	switch (cmd) {
		...
		case ECS_IOCTL_APP_GET_MVFLAG:
		case ECS_IOCTL_APP_GET_DELAY:
			if (copy_to_user(argp, cachedData->delay, AKM_DELAY_LENGTH)) {
				return -EFAULT;
			}
			break;
		default:
...
int sensor_probe(struct i2c_client *client, const struct i2c_device_id *devid)
{
	...
	switch(type)
	{
		...
		case SENSOR_TYPE_COMPASS:	
			...
			/* status of rotation sensor */
			input_set_abs_params(sensor->input_dev, ABS_RUDDER, 0, 3, 0, 0);

			/* x-axis acceleration (720 x 16G) */
			input_set_abs_params(sensor->input_dev, ABS_X, -11520, 11520, 0, 0);
			/* y-axis acceleration (720 x 16G) */
			input_set_abs_params(sensor->input_dev, ABS_Y, -11520, 11520, 0, 0);
			/* z-axis acceleration (720 x 16G) */
			input_set_abs_params(sensor->input_dev, ABS_Z, -11520, 11520, 0, 0);
			/* status of acceleration sensor */
			input_set_abs_params(sensor->input_dev, ABS_WHEEL, 0, 3, 0, 0);

			/* x-axis of raw magnetic vector (-4096, 4095) */
			input_set_abs_params(sensor->input_dev, ABS_HAT0X, -32768, 32767, 0, 0);
			/* y-axis of raw magnetic vector (-4096, 4095) */
			input_set_abs_params(sensor->input_dev, ABS_HAT0Y, -32768, 32767, 0, 0);
			/* z-axis of raw magnetic vector (-4096, 4095) */
			input_set_abs_params(sensor->input_dev, ABS_BRAKE, -32768, 32767, 0, 0);
			/* status of magnetic sensor */
			input_set_abs_params(sensor->input_dev, ABS_HAT1X, 0, 3, 0, 0);
			break;		
		case SENSOR_TYPE_GYROSCOPE:
	...
	//<-- ASUS-Bevis_chen + -->
	#ifdef CONFIG_ASUS_ENGINEER_MODE
	if(setup_asusproc_sensorstatus_entry(type))
		printk("%s: ERROR to create sensor proc entry\n",__func__);
	#endif
	//<-- ASUS-Bevis_chen - -->
...
static void sensor_shut_down(struct i2c_client *client)
{
	...
	if(sensor->status_cur == SENSOR_ON)
	{
		if ( sensor->ops->active(client, SENSOR_OFF, 0) < 0 )
		{
			mutex_unlock(&sensor->operation_mutex);
			return;
		}
