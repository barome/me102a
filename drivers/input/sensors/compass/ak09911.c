...
{	
	...
	memset(cachedData, 0, sizeof(struct ECS_cached_data));
	this_client = client;	
...
static int get_data(struct i2c_client *client)
{
	struct sensor_private_data *sensor =
	    	(struct sensor_private_data *) i2c_get_clientdata(client);	
	...
	memcpy(cachedData->compass_data, buffer, sensor->ops->read_len);
...
static void compass_set_YPR(int *rbuf)
{
	...
	if (atomic_read(&sensor->flags.m_flag) && (rbuf[0] & ORI_DATA_READY)) {
		...
		DBG("%s:m_flag:x=%d,y=%d,z=%d,RUDDER=%d\n",__func__,rbuf[9], rbuf[10], rbuf[11], rbuf[8]);
	}
...
static int compass_akm_reset(struct i2c_client *client)
{
	...
	{
		/* soft reset */
		result = sensor_write_reg(client, AK09911_REG_CNTL3, AK09911_RESET_DATA);
		if(result)
			  ...
			  unsigned int cmd, unsigned long arg)
{
	...
	case ECS_IOCTL_GETDATA:
			...
			get_data(client);
			mutex_lock(&sensor->data_mutex);	
			memcpy(compass_data, cachedData->compass_data, SENSOR_DATA_SIZE);	//get data from buffer
			mutex_unlock(&sensor->data_mutex);
			break;
	case ECS_IOCTL_SET_YPR: 		
	...
	case ECS_IOCTL_GET_DELAY:
		...
		mutex_lock(&sensor->operation_mutex);
                memcpy(delay, cachedData->delay, AKM_DELAY_LENGTH);
		mutex_unlock(&sensor->operation_mutex);
		break;
