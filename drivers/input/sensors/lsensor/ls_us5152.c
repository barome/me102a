...
static int sensor_active(struct i2c_client *client, int enable, int rate)
{
	...
	if(enable)
	{
		sensor_write_reg(client, 0x00, 0x90);//operation;continuous;ALS only
		sensor_write_reg(client, 0x21, 0x01);//Mode Storage
		sensor_write_reg(client, 0x01, 0x13);//ALS_FQ once;ALS 16Bit;ALS Gain x8
		sensor_write_reg(client, 0x03, 0x04);//No wait time;INT pin for ALS
		sensor_write_reg(client, 0x10, 0x01);//50/60Hz rejection enable
		sensor_write_reg(client, 0x16, 0x00);//Dark Offset Gain
		
		sensor_write_reg(client, 0x04, 0x00);//ALS Low-Threadhold lower byte AL[7:0]
		sensor_write_reg(client, 0x05, 0x00);//ALS Low-Threshold upper byte AL[15:8]
		sensor_write_reg(client, 0x06, 0xff);//ALS High-Threshold lower byte AH[7:0]
		sensor_write_reg(client, 0x07, 0xff);//ALS High-Threshold upper byte AH[15:8]
		sensor_write_reg(client, 0x00, 0x91);//Word Mode(Read 0x0C,0x0D in word mode)
	}
	else
	{
		value = sensor_read_reg(client, REGS_CR0);
		value |= PWR_MODE_DOWN_MASK;
...
static int sensor_report_value(struct i2c_client *client)
{
	...
	do
	{
		...
		sensor_write_reg(client, sensor->ops->ctrl_reg, ctrl_data | CR0_ONESHOT_MASK);
