...
static int rx_irq = 0;
static struct wake_lock rx_bt_wakelock;
static irqreturn_t bt_rx_wakeup_host(int irq, void *dev)
{
    printk("%s.\n", __func__);
    iomux_set(UART0_SIN);
    wake_lock_timeout(&rx_bt_wakelock, 3 * HZ);
    return 0;
}

static int uart0_rx_setup_irq(struct uart_rk_port *up)
{
    int ret = 0;
    if(up->port.line == 0)
    {
       wake_lock_init(&rx_bt_wakelock, WAKE_LOCK_SUSPEND, "rx_bt_wake");
       iomux_set(GPIO1_A0);
       ret = gpio_request(RK30_PIN1_PA0, "uart0_rx_gpio");
       if (ret) {
           printk("request uart0_rx_gpio fail ! \n");
           return -1;
       }

       gpio_direction_input(RK30_PIN1_PA0);
       rx_irq = gpio_to_irq(RK30_PIN1_PA0);
       ret = request_irq(rx_irq, bt_rx_wakeup_host, IRQF_TRIGGER_FALLING, "bt_rx_wakeup",NULL);
       if(ret < 0)
       {
           printk("bt_host_wake rx_irq request fail.\n");
           return ret;
       }
       enable_irq_wake(rx_irq);
       disable_irq_nosync(rx_irq);
       iomux_set(UART0_SIN);
    }
    return ret;
}

static int uart0_rx_enable_irq(struct uart_rk_port *up)
{
    if(up->port.line == 0)
    {
        iomux_set(GPIO1_A0);
...
static int uart0_rx_disable_irq(struct uart_rk_port *up)
{
    if(up->port.line == 0)
    {
        disable_irq_nosync(rx_irq);
...
static int uart0_rx_remove_irq(struct uart_rk_port *up)
{
    if(up->port.line == 0)
    {
        disable_irq_nosync(rx_irq);
        ...
        wake_lock_destroy(&rx_bt_wakelock);
    }
    return 0;
}
