...
static int asus_pmu_procfile_read(char *buffer, char **buffer_location,
...
int create_asusproc_pmu_status_entry( void )
{   
    struct proc_dir_entry *res;
    res = create_proc_entry("asus_pmu_status", S_IWUGO| S_IRUGO, NULL);
    if (!res)
        return -ENOMEM;
    res->read_proc = asus_pmu_procfile_read;
    return 0;
}
...
static int __devinit act8846_i2c_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	//<-- ASUS-Bevis_chen + -->
	#ifdef CONFIG_ASUS_ENGINEER_MODE
	if(create_asusproc_pmu_status_entry( ))
		printk("%s: ERROR to create pmu proc entry\n",__func__);
	#endif
	//<-- ASUS-Bevis_chen - -->
