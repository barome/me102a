...
int rk29_bl_val_scalor_conic(struct rk29_bl_info *rk29_bl_info,int brightness)
{
	...
	if(rk29_bl_info->max_brightness<rk29_bl_info->min_brightness)
		...
		temp = 243*(brightness + 38)*(brightness + 38);
		brightness = temp/293/293;
	#else
		if(brightness<=40)
		{
			if(brightness<rk29_bl_info->min_brightness)
				brightness=rk29_bl_info->min_brightness;
			brightness = (9-rk29_bl_info->min_brightness)*brightness;
			brightness = (brightness/(40-rk29_bl_info->min_brightness)) + rk29_bl_info->min_brightness;
		}else{
