...
#define OUT_CLK	         124*1000000  //50Hz
...
static unsigned int pre_initialize[] = {
	...
	0x00B40000 | H_VD,
	0x00B50000 | V_VD,
	0x00B6000b,     // burst mode 24bits
	
	0x00de0000 | (data_lane -1),    //4 lanes
	0x00d60004,
	
	0x00B90000,
	0x00bac022,   //pll    //816MHz
	0x00Bb000a,
	0x00B90001,	
	0x00c40001,
...
int lcd_standby(u8 enable)
{
	if(enable) {
		...
		dsi_set_regs(disable_video_mode, ARRAY_SIZE(disable_video_mode));
		msleep(100);
		dsi_power_off();
