...
static void rk_camera_capture_process(struct work_struct *work)
{
    ...
    if (pcdev->hdr_info.en) {
        for (i=0; i<3; i++) {
            if (pcdev->hdr_info.frame[i].set_ts == 0) {
                if (i==0) {
                    v4l2_subdev_call(sd, core, ioctl, RK29_CAM_SUBDEV_HDR_EXPOSURE,(void*)pcdev->hdr_info.frame[i].code);
                    pcdev->hdr_info.frame[i].set_ts = pcdev->fps;
                    printk("set hdr %d @ %d fps\n",i,pcdev->fps);
                }
            } else if (pcdev->hdr_info.frame[i].get_ts == 0) {
                if (camera_work->ts > pcdev->hdr_info.frame[i].set_ts) {
                    if ((camera_work->ts - pcdev->hdr_info.frame[i].set_ts)>=5) {
                        printk("get hdr %d @ %d fps\n",i,camera_work->ts);
                        pcdev->hdr_info.frame[i].get_ts = camera_work->ts;
                        RK_VIDEOBUF_CODE_SET(camera_work->vb->rk_code,pcdev->hdr_info.frame[i].code); 
                        if ((i<2) && (pcdev->hdr_info.frame[i+1].set_ts == 0)) {
                            v4l2_subdev_call(sd, core, ioctl, RK29_CAM_SUBDEV_HDR_EXPOSURE,(void*)pcdev->hdr_info.frame[i+1].code);
                            pcdev->hdr_info.frame[i+1].set_ts = pcdev->fps;
                            printk("set hdr %d @ %d fps\n",i+1,pcdev->fps);
                        } 
...
static int rk_camera_mclk_control(struct soc_camera_device *icd, int on){
	struct soc_camera_host *ici = to_soc_camera_host(icd->dev.parent);
	struct rk_camera_dev *pcdev = ici->priv;
	static bool clkStatus = false;
	if(!pcdev->cif_clk_out){
		RK30_CAM_DEBUG_TRACE(KERN_ERR "failed to get cif clock source\n");
		return -1;
	}
	if(on && !clkStatus){
		clkStatus = true;
	...
	}else if(!on && clkStatus){
		clkStatus = false;
		clk_disable(pcdev->cif_clk_out);
		clk_enable(pcdev->cif_clk_out);
		clk_set_rate(pcdev->cif_clk_out,RK_SENSOR_48MHZ);
		clk_disable(pcdev->cif_clk_out);
	}
	return 0;
}
...
static void rk_camera_deactivate(struct rk_camera_dev *pcdev)
{
	...
	if(pcdev->icd)
		rk_camera_mclk_control(pcdev->icd,0);
                                ...
                                struct v4l2_capability *cap)
{
    ...
    if (orientation[0] != '-') {
        RK30_CAM_DEBUG_TRACE("%s: %s is not registered in rk29_camera_platform_data, orientation apply default value",__FUNCTION__,dev_name(pcdev->icd->pdev));
...
static int rk_camera_probe(struct platform_device *pdev)
{
    ...
    if(IS_ERR(pcdev->pd_cif) || IS_ERR(pcdev->aclk_cif) || IS_ERR(pcdev->hclk_cif) || IS_ERR(pcdev->cif_clk_in) || IS_ERR(pcdev->cif_clk_out)){
        RK30_CAM_DEBUG_TRACE(KERN_ERR "%s(%d): failed to get cif clock source\n",__FUNCTION__,__LINE__);
        err = -ENOENT;
	...
	pcdev->pdata->rk_host_mclk_ctrl = rk_camera_mclk_control;
