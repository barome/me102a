...
static const struct v4l2_querymenu ov5693_sensor_menus[] =
{
...
static	struct v4l2_queryctrl ov5693_sensor_controls[] =
{
	#if CONFIG_SENSOR_WhiteBalance
	{
...
static int ov5693_before_init_cb(const struct i2c_client *client)
{
	return icatch_update_sensor_ops(
			ov5693_sensor_controls,
			ARRAY_SIZE(ov5693_sensor_controls),
			ov5693_sensor_menus,
			ARRAY_SIZE(ov5693_sensor_menus));
}

static int ov5693_sensor_probe(struct i2c_client *client,
...
//	int ret;
	...
	sensor->before_init_cb = ov5693_before_init_cb;
	if ((icatch_get_rearid_by_lowlevelmode(icd, &sensorid) != 0) || sensorid != 0x4800) {
		kfree(sensor);
...
static int ov5693_sensor_remove(struct i2c_client *client)
{
...
static struct i2c_driver sensor_i2c_driver = {
	.driver = {
			.name = SENSOR_REAR_NAME_STRING(),
	},
	.probe  = ov5693_sensor_probe,
	.remove  = ov5693_sensor_remove,
	.id_table = sensor_id,
};
 
static int __init sensor_mod_init(void)
{
	DEBUG_TRACE("\n%s..%s.. \n",__FUNCTION__,SENSOR_REAR_NAME_STRING());
...
MODULE_DESCRIPTION(SENSOR_REAR_NAME_STRING(Camera sensor driver));
