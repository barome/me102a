...
#ifdef CONFIG_DEBUG
{
	...
	if (rtw_is_list_empty(phead) == _FALSE)
	{
		struct cmd_obj *obj;
		...
		pcmd_list = phead;
		do {
			pcmd_list = get_next(pcmd_list);
			if (rtw_end_of_queue_search(phead, pcmd_list) == _TRUE)
				break;

			obj = LIST_CONTAINOR(pcmd_list, struct cmd_obj, list);
			if (obj->cmdcode == GEN_CMD_CODE(_Set_Drv_Extra))
			{
				pdrvextra_cmd_parm = (struct drvextra_cmd_parm*)obj->parmbuf;
				DBG_871X(FUNC_ADPT_FMT ": [WARNING] DRVEXTRA(%d) is in cmd queue and will be DROPPED!\n",
					FUNC_ADPT_ARG(padapter), pdrvextra_cmd_parm->ec_id);
			}
			else
			{
				DBG_871X(FUNC_ADPT_FMT ": [WARNING] CMD(%d) is in cmd queue and will be DROPPED!\n",
					FUNC_ADPT_ARG(padapter), obj->cmdcode);
			}
		} while (1);
	}
	else
	{
		DBG_871X(FUNC_ADPT_FMT ": Nothing left in cmd queue after hal deinit\n",
			FUNC_ADPT_ARG(padapter));
	}
	_exit_critical(&queue->lock, &irqL);

	phead = &padapter->xmitpriv.pending_xmitbuf_queue.queue;
	if (rtw_is_list_empty(phead) == _FALSE)
	{
		DBG_871X(FUNC_ADPT_FMT ": [WARNING] Something left in xmitbuf queue after hal deinit!!\n",
			FUNC_ADPT_ARG(padapter));
	}
	else
	{
		DBG_871X(FUNC_ADPT_FMT ": Nothing left in xmitbuf queue after hal deinit\n",
			FUNC_ADPT_ARG(padapter));
	}
	...
	if (padapter->pbuddy_adapter)
	{
		...
		pbuddy = padapter->pbuddy_adapter;

		phead = &pbuddy->cmdpriv.cmd_queue.queue;
		if (rtw_is_list_empty(phead) == _FALSE)
		{
			DBG_871X(FUNC_ADPT_FMT ": [WARNING] Something left in cmd queue after hal deinit!!\n",
				FUNC_ADPT_ARG(pbuddy));
		}

		phead = &pbuddy->xmitpriv.pending_xmitbuf_queue.queue;
		if (rtw_is_list_empty(phead) == _FALSE)
		{
			DBG_871X(FUNC_ADPT_FMT ": [WARNING] Something left in xmitbuf queue after hal deinit!!\n",
				FUNC_ADPT_ARG(pbuddy));
