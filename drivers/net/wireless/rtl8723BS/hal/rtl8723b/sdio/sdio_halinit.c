...
//#endif // CONFIG_PLATFORM_SPRD
	...
	DBG_8192C("%s: REG_SYS_FUNC_EN(0x%X)=0x%02X\n", __FUNCTION__, REG_SYS_FUNC_EN, rtw_read8(padapter, REG_SYS_FUNC_EN));
	...
	// 0x4c[23] = 0x1
	// 0x64[0] = 0
	value16 = rtw_read16(padapter, 0x38);
	value16 |= BIT(11);
	rtw_write16(padapter, 0x38, value16);
	DBG_8192C("%s: 0x38=0x%04X\n", __FUNCTION__, rtw_read16(padapter, 0x38));

	value32 = rtw_read32(padapter, 0x4c);
	value32 |= BIT(23);
	rtw_write32(padapter, 0x4c, value32);
	DBG_8192C("%s: 0x4c=0x%08X\n", __FUNCTION__, rtw_read32(padapter, 0x4c));

	value8 = rtw_read8(padapter, 0x64);
	value8 &= ~BIT(0);
	rtw_write8(padapter, 0x64, value8);
	DBG_8192C("%s: 0x64=0x%02X\n", __FUNCTION__, rtw_read8(padapter, 0x64));
...
void _InitRateFallback(PADAPTER padapter)
{
	...
	rtw_write32(padapter, REG_DARFRC+4, 0x02020101);//suggested by SD3-Stanley
...
static void CardDisableRTL8723BSdio(PADAPTER padapter)
{
	...
	if ((adapter_to_pwrctl(padapter)->bInSuspend == _TRUE)
		|| (adapter_to_pwrctl(padapter)->bips_processing == _TRUE))
	{
		DBG_871X("%s: run CARD DISABLE flow\n", __FUNCTION__);
		ret = HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_SDIO_MSK, rtl8723B_card_disable_flow);
		if (ret == _FALSE) {
			DBG_8192C("%s: run CARD DISABLE flow fail!\n", __FUNCTION__);
		}
	}
	else
	{
		// Power down.
		DBG_871X("%s: run HW PWRDN\n", __FUNCTION__);
		ret = HalPwrSeqCmdParsing(padapter, PWR_CUT_ALL_MSK, PWR_FAB_ALL_MSK, PWR_INTF_SDIO_MSK, rtl8723B_hwpdn_flow);
		if (ret == _FALSE) {
			DBG_8192C("%s: run CARD POWER DOWN flow fail!\n", __FUNCTION__);
		}
	}
}
