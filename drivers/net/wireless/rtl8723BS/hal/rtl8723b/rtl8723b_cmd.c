...
_func_enter_;
	...
	DBG_871X(FUNC_ADPT_FMT ": mac_id=%d raid=0x%x bw=%d mask=0x%x\n",
		FUNC_ADPT_ARG(padapter), mac_id, raid, bw, mask);
	...
	{
		// IOT with Belkin AC1200
//		DBG_871X("Disable PT for Belkin AP \n");
...
_func_enter_;
...
#ifdef CONFIG_WOWLAN
	if(psmode == PS_MODE_SELF_DEFINED)  //For WOWLAN LPS, DTIM = (awake_intvl - 1)
	{
		awake_intvl = 4;
		rlbm = 2;
	}	
	...
	SET_8723B_H2CCMD_PWRMODE_PARM_SMART_PS(u1H2CPwrModeParm, 0); // for ASUS RK3188
...
_func_enter_;
	...
	DBG_8192C("+" FUNC_ADPT_FMT ": iface_type=%d mstatus(%x)\n",
		FUNC_ADPT_ARG(padapter), padapter->iface_type, mstatus);
