...
char *rtw_fw_file_path = "/system/etc/firmware/rtlwifi/FW_NIC.BIN"; // for ASUS RK3188
module_param(rtw_fw_file_path, charp, 0644);
MODULE_PARM_DESC(rtw_fw_file_path, "The path of fw image");

char *rtw_fw_wow_file_path = "/system/etc/firmware/rtlwifi/FW_WoWLAN.BIN"; // for ASUS RK3188
...
char *rtw_fw_mp_bt_file_path = "/system/etc/firmware/rtlwifi/FW_MP_BT.BIN"; // for ASUS RK3188
...
void rtw_proc_init_one(struct net_device *dev)
{
	...
	entry = create_proc_read_entry("rx_ampdu", S_IFREG | S_IRUGO,
				   dir_dev, proc_get_rx_ampdu, dev);
	if (!entry) {
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	}
	entry->write_proc = proc_set_rx_ampdu;
	
	entry = create_proc_read_entry("en_fwps", S_IFREG | S_IRUGO,
				   dir_dev, proc_get_en_fwps, dev);
	if (!entry) {
		DBG_871X("Unable to create_proc_read_entry!\n");
		return;
	}
	entry->write_proc = proc_set_en_fwps;
...
void rtw_proc_remove_one(struct net_device *dev)
{
	...
	if (dir_dev) {
		...
		remove_proc_entry("rx_ampdu", dir_dev);
		
		remove_proc_entry("en_fwps", dir_dev);
...
int rtw_ips_pwr_up(_adapter *padapter)
{
	...
	XXX: //This case is not handled. It shall be considered with the following silent_reset_inprogress!
