...
#ifdef CONFIG_LPS
		traffic_check_for_leave_lps(padapter, _FALSE);
#endif //CONFIG_LPS
...
int recv_func_prehandle(_adapter *padapter, union recv_frame *rframe)
{
	...
	if (padapter->registrypriv.mp_mode == 1)
	{
		...
		if (pattrib->crc_err == 1)
			...
			{	
				if(padapter->mppriv.bSetRxBssid == _TRUE){
					pda = get_da(ptr);
					_rtw_memcpy(pattrib->dst, pda, ETH_ALEN);
					
					if(_rtw_memcmp(pattrib->dst,padapter->mppriv.network_macaddr,ETH_ALEN ) ) {
							padapter->mppriv.rx_bssidpktcount++;
							DBG_8192C("%s , pattrib->dst =%x %x %x %x %x %x\n",__func__,
