...
#endif // CONFIG_AUTH_DIRECT_WITHOUT_BCN
...
*/
	...
	pdrvextra_cmd_parm->type_size = 0;
	pdrvextra_cmd_parm->pbuf = (u8 *)padapter; //only  primary padapter does this cmd
...
#define TRAFFIC_PROTECT_PERIOD_MS 4500
	...
	if (link_detect->NumTxOkInPeriod > TX_ACTIVE_TH
		...
		if (
#if 0
			((pmlmepriv->LinkDetectInfo.NumRxUnicastOkInPeriod + pmlmepriv->LinkDetectInfo.NumTxOkInPeriod) > 8) ||
			(pmlmepriv->LinkDetectInfo.NumRxUnicastOkInPeriod > 2)
#else // for ASUS RK3188
			((pmlmepriv->LinkDetectInfo.NumRxUnicastOkInPeriod + pmlmepriv->LinkDetectInfo.NumTxOkInPeriod) > 8 ) ||
			(pmlmepriv->LinkDetectInfo.NumRxUnicastOkInPeriod > 2/*4*/) ||
			(pmlmepriv->LinkDetectInfo.NumTxOkInPeriod > 4)
#endif // for ASUS RK3188
			)
		{
...
void dynamic_chk_wk_hdl(_adapter *padapter, u8 *pbuf, int sz)
{
	...
	if(is_primary_adapter(padapter))
		printk("0xFD=%x, 0xFE=%x, 0xFF=%x\n", rtw_read8(padapter, 0xFD), rtw_read8(padapter, 0xFE), rtw_read8(padapter, 0xFF));
...
u8 rtw_drvextra_cmd_hdl(_adapter *padapter, unsigned char *pbuf)
{
	...
	switch(pdrvextra_cmd->ec_id)
	{
		case DYNAMIC_CHK_WK_CID://only  primary padapter go to this cmd, but execute dynamic_chk_wk_hdl() for two interfaces
			...
			{
				pdrvextra_cmd->pbuf = (u8 *)padapter->pbuddy_adapter;
