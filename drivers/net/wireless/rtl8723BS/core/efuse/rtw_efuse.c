	...
	IN	BOOLEAN		bPseudoTest)
{
	...
	{	
		...
		PHY_SetMacReg(pAdapter, REG_SYS_ISO_CTRL, BIT14, 1); // Turn on power cut
		Efuse_PowerSwitch(pAdapter, _TRUE, _TRUE); // LDO 2.6V, this must follows after power cut.
	...
	if (IS_HARDWARE_TYPE_8723B(pAdapter))
	{
		// <20130121, Kordan> For SMIC EFUSE specificatoin.
		// 0x34[11]: SW force PGMEN input of efuse to high. (for the bank selected by 0x34[9:8])
		PHY_SetMacReg(pAdapter, EFUSE_TEST, BIT(11), 1);
	...
	{
		PHY_SetMacReg(pAdapter, EFUSE_TEST, BIT(11), 0);
		Efuse_PowerSwitch(pAdapter,TRUE, FALSE); // Turn off LDO 2.6V first. 
	}
