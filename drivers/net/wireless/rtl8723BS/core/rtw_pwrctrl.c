...
#ifdef CONFIG_LPS
void	traffic_check_for_leave_lps(PADAPTER padapter, u8 tx)
{
#ifdef CONFIG_CHECK_LEAVE_LPS
	u8	bLeaveLPS = _FALSE;
	struct mlme_priv	*pmlmepriv = &padapter->mlmepriv;

	if(tx) // checking leave_lps at caller
	{
		bLeaveLPS = _TRUE;
	}
	else
	{
		if(pmlmepriv->LinkDetectInfo.NumRxUnicastOkInPeriod > 2)
			bLeaveLPS = _TRUE;	
	}	
	...
	if(bLeaveLPS)
	{
		...
		{
			DBG_871X("leave lps via %s, Tx = %d, Rx = %d \n", tx?"Tx":"Rx", pmlmepriv->LinkDetectInfo.NumTxOkInPeriod,pmlmepriv->LinkDetectInfo.NumRxOkInPeriod);			
...
_func_enter_;
	...
	//rtw_register_early_suspend(pwrctrlpriv); for ASUS WOW.
...
_func_enter_;
	...
	#if defined(CONFIG_HAS_EARLYSUSPEND) || defined(CONFIG_ANDROID_POWER)
	//rtw_unregister_early_suspend(pwrctrlpriv); for ASUS WOW.
	#endif //CONFIG_HAS_EARLYSUSPEND || CONFIG_ANDROID_POWER
