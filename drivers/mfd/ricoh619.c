...
//	__ricoh618_write(ricoh618_i2c_client, RICOH618_PWR_SLP_CNT, 0x1); //Power OFF
	...
	ricoh619_read(ricoh619->dev, 0xBD, &status);//<Sherwin_Yan@asus.com--20130723> reboot to charger mode when usb or AC is on
       if(status&0xc0)
...
#ifdef CONFIG_PM
static void Clear_Interrupt_Flage(struct i2c_client *client)
{
	uint8_t reg_val;

	 __ricoh619_read(client, RICOH619_INTC_INTMON, &reg_val);
	if(reg_val!=0)
	{
		printk("======suspend factor error\n");
		 __ricoh619_write(client, 0x13, 0);//pwrirq
		 
		 //__ricoh619_write(client, 0x41, 0);//dcirq dcdc limit

		 //__ricoh619_write(client, 0xaf, 0);//rtccnt2 bit0
		
		 __ricoh619_write(client, 0x8c, 0);//ir_adc1
		 __ricoh619_write(client, 0x8d, 0);//ir_adc2
		 __ricoh619_write(client, 0x8e, 0);//ir_adc3

		//__ricoh619_write(client, 0x95, 0);//ir_gpr
		//__ricoh619_write(client, 0x96, 0);//ir_gpr

		__ricoh619_write(client, 0xc2, 0);//chgctrl_irq
		__ricoh619_write(client, 0xc3, 0);//chgctrl_irq1
		__ricoh619_write(client, 0xc4, 0);//chgctrl_irq2
		__ricoh619_write(client, 0xc5, 0);//chgerr_irr
		__ricoh619_write(client, 0xd2, 0);//chgextif_irr		
	}	
}
...
static int ricoh619_i2c_suspend(struct i2c_client *client, pm_message_t state)
{
	if (client->irq)
	 ...
	 //__ricoh619_write(client, RICOH619_INTC_INTEN, 0); //ken ding bu neng guan ,rtc,key deng xu zhi qi intb
	 Clear_Interrupt_Flage(client);
...
//		printk("PMU: %s: PWR_KEY Wakeup\n",__func__);
	...
	__ricoh619_read(client, RICOH619_INTC_INTMON, &reg_val);
	printk("INTMON:%d\n",reg_val);
	__ricoh619_read(client, RICOH619_INTC_INTEN, &reg_val);
	__ricoh619_write(client, RICOH619_INTC_INTEN, 0);
	//Clear_Interrupt_Flage(client);
	enable_irq(client->irq);
	__ricoh619_write(client, RICOH619_INTC_INTEN, reg_val);
	return 0;
}
