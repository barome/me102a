...
#include <linux/proc_fs.h>
static int asus_codec_procfile_read(char *buffer, char **buffer_location,
...
int create_asusproc_codec_status_entry( void )
{   
    struct proc_dir_entry *res;
    res = create_proc_entry("asus_codec_status", S_IWUGO| S_IRUGO, NULL);
    if (!res)
        return -ENOMEM;
    res->read_proc = asus_codec_procfile_read;
    return 0;
}
#endif
//<ASUS-Bevis_chen ->
...
static struct rt5639_init_reg init_list[] = {
	...
	{RT5639_PRIV_INDEX	, 0x006e},//PR6e = 3199'h
	{RT5639_PRIV_DATA	, 0x3199},
	/*playback*/
	...
	{RT5639_HP_VOL		, 0x8c8c},//OUTMIX -> HPVOL
...
//	{RT5639_HP_CALIB_AMP_DET, 0x0420},
	...
	//xin_jin@asus.com++ for volume
	{RT5639_SPK_VOL		, 0x0505},//SPKMIX -> SPKVOL   0x8b8b->0x8888
	//xin_jin@asus.com--
	{RT5639_SPO_CLSD_RATIO	, 0x0006},
...
//	{RT5639_I2S1_SDP	, 0xD000},//change IIS1 and IIS2
	/*record*/
	//terry_tao@asus.com++ modify for MIC
//	{RT5639_IN1_IN2		, 0x5080},//IN1 boost 40db and differential mode
	{RT5639_IN1_IN2		, 0x0800},//IN3 boost 40db and signal ended mode
	{RT5639_IN3_IN4		, 0x0400},//IN2 boost 40db and signal ended mode
//	{RT5639_REC_L2_MIXER	, 0x007d},//Mic1 -> RECMIXL
//	{RT5639_REC_R2_MIXER	, 0x007d},//Mic1 -> RECMIXR
	{RT5639_REC_L2_MIXER	, 0x006b},//Mic2 -> RECMIXL
	{RT5639_REC_R2_MIXER	, 0x006b},//Mic2 -> RECMIXR
	//terry_tao@asus.com--
...
static void rt5639_headphone_unmute_sequence(struct snd_soc_codec *codec)
{
...
static void rt5639_headphone_mute_sequence(struct snd_soc_codec *codec)
{
	snd_soc_update_bits(codec, RT5639_DEPOP_M3,
		RT5639_CP_FQ1_MASK | RT5639_CP_FQ2_MASK | RT5639_CP_FQ3_MASK,
	...
	msleep(30);
	snd_soc_update_bits(codec, RT5639_DEPOP_M1,
		RT5639_HP_R_SMT_MASK | RT5639_HP_L_SMT_MASK,
		RT5639_HP_L_SMT_DIS | RT5639_HP_R_SMT_DIS);
}

static const char *rt5639_playback_path_mode[] = {"OFF", "RCV", "SPK", "HP", "HP_NO_MIC", "BT", "SPK_HP", //0-6
													"RING_SPK", "RING_HP", "RING_HP_NO_MIC", "RING_SPK_HP"};//7-10

static const SOC_ENUM_SINGLE_DECL(rt5639_path_type, 0, 0, rt5639_playback_path_mode);

static int rt5639_playback_path_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5639_priv *rt5639 = snd_soc_codec_get_drvdata(codec);
	...
	ucontrol->value.integer.value[0] = rt5639->playback_path;

	return 0;
}

static int rt5639_playback_path_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5639_priv *rt5639 = snd_soc_codec_get_drvdata(codec);

	if (rt5639->playback_path == ucontrol->value.integer.value[0]){
		printk("%s:playback_path is not changed!\n",__func__);
		//return 0;
	}

	rt5639->playback_path = ucontrol->value.integer.value[0];
	printk("%s:set playback_path=%ld\n",__func__,rt5639->playback_path);
	
	switch (rt5639->playback_path) {
	case OFF:
		//rt5639_headphone_mute_sequence(codec);
	...
	case RING_SPK:
		//rt5639_headphone_mute_sequence(codec);
		snd_soc_write(codec, RT5639_DAC1_DIG_VOL, 0xafae);
		mutex_lock(&codec->mutex);
		snd_soc_dapm_disable_pin(&codec->dapm, "Headphone Jack");
	...
	case RING_HP:
	case RING_HP_NO_MIC:
		snd_soc_write(codec, RT5639_DAC1_DIG_VOL, 0xafaf);
		mutex_lock(&codec->mutex);
		snd_soc_dapm_disable_pin(&codec->dapm, "Ext Spk");
		snd_soc_dapm_enable_pin(&codec->dapm, "Headphone Jack");
		snd_soc_dapm_sync(&codec->dapm);
		mutex_unlock(&codec->mutex);
		//rt5639_headphone_unmute_sequence(codec);
		break;
...
static const char *rt5639_capture_path_mode[] = {"MIC OFF", "Main Mic", "Hands Free Mic", "BT Sco Mic"};//

static const SOC_ENUM_SINGLE_DECL(rt5639_capture_type, 0, 0, rt5639_capture_path_mode);

static int rt5639_capture_path_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5639_priv *rt5639 = snd_soc_codec_get_drvdata(codec);
	...
	ucontrol->value.integer.value[0] = rt5639->capture_path;

	return 0;
}

static int rt5639_capture_path_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	struct rt5639_priv *rt5639 = snd_soc_codec_get_drvdata(codec);

	if (rt5639->capture_path == ucontrol->value.integer.value[0]){
		printk("%s:capture_path is not changed!\n",__func__);
		//return 0;
	}

	rt5639->capture_path = ucontrol->value.integer.value[0];
	printk("%s:set capture_path=%ld\n",__func__,rt5639->capture_path);
	switch (rt5639->capture_path) {
	case MIC_OFF:
		snd_soc_write(codec, RT5639_ADC_DIG_VOL, 0x4646);
		snd_soc_write(codec, RT5639_DRC_AGC_2, 0x1f00);
		snd_soc_write(codec, RT5639_DRC_AGC_3, 0x0000);
		snd_soc_write(codec, RT5639_DRC_AGC_1, 0x2206);
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER, 
			RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L, RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L);//bit2 = main mic, bit4 = headset mic;
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L, RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L);
		break;
	
	case Main_Mic:
		snd_soc_write(codec, RT5639_ADC_DIG_VOL, 0x4646);
		snd_soc_write(codec, RT5639_DRC_AGC_2, 0x1e00);
		snd_soc_write(codec, RT5639_DRC_AGC_3, 0x0080);
		snd_soc_write(codec, RT5639_DRC_AGC_1, 0xe206);
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER, 
			RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L, RT5639_M_BST4_RM_L | 0);
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L, RT5639_M_BST4_RM_L | 0);
		break;

	case Hands_Free_Mic:
		snd_soc_write(codec, RT5639_ADC_DIG_VOL, 0x3c3c);
		snd_soc_write(codec, RT5639_DRC_AGC_2, 0x1f00);
		snd_soc_write(codec, RT5639_DRC_AGC_3, 0x0000);
		snd_soc_write(codec, RT5639_DRC_AGC_1, 0x2206);
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER, 
			RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L, 0 | RT5639_M_BST2_RM_L);
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST4_RM_L | RT5639_M_BST2_RM_L, 0 | RT5639_M_BST2_RM_L);
		break;
...
//bard 8-9 s
static const char *rt5639_mic_mode[] = {"off", "on",};
static const SOC_ENUM_SINGLE_DECL(rt5639_mic_enum, 0, 0, rt5639_mic_mode);
//bard 8-9 e
...
static const SOC_ENUM_SINGLE_DECL(
	rt5639_in3_mode_enum, RT5639_IN1_IN2,
...
//bard 8-9 s
static int rt5639_mic1_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	DBG("%s:%d\n",__func__,__LINE__);
	...
	ucontrol->value.integer.value[0] = 
		(snd_soc_read(codec, RT5639_REC_L2_MIXER) & RT5639_M_BST2_RM_L) >> RT5639_M_BST2_RM_L_SFT;

	return 0;
}

static const char *rt5639_hp_mute_mode[] = {"off", "on",};

static const SOC_ENUM_SINGLE_DECL(rt5639_hp_mute_enum, 0, 0, rt5639_hp_mute_mode);

static int rt5639_mic1_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	DBG("%s:%d\n",__func__,__LINE__);
	...
	if(ucontrol->value.integer.value[0]==0) {
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER,
			RT5639_M_BST2_RM_L, 0);
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST2_RM_R, 0);
	}else {
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER,
			RT5639_M_BST2_RM_L, RT5639_M_BST2_RM_L);
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST2_RM_R, RT5639_M_BST2_RM_R);
	}

	return 0;
}

static int rt5639_mic2_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	DBG("%s:%d\n",__func__,__LINE__);
	...
	ucontrol->value.integer.value[0] = 
		(snd_soc_read(codec, RT5639_REC_L2_MIXER) & RT5639_M_BST4_RM_L) >> RT5639_M_BST4_RM_L_SFT;

	return 0;
}

static int rt5639_mic2_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	DBG("%s:%d\n",__func__,__LINE__);
	...
	if(ucontrol->value.integer.value[0]==0) {
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER,
			RT5639_M_BST4_RM_L, 0);
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST4_RM_R, 0);
	}else {
		snd_soc_update_bits(codec, RT5639_REC_L2_MIXER,
			RT5639_M_BST4_RM_L, RT5639_M_BST4_RM_L);
		snd_soc_update_bits(codec, RT5639_REC_R2_MIXER,
			RT5639_M_BST4_RM_R, RT5639_M_BST4_RM_R);
	}
...
static int rt5639_hp_mute_get(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	 
	ucontrol->value.integer.value[0] = 
		!((snd_soc_read(codec, RT5639_HP_VOL) & RT5639_L_MUTE) >> RT5639_L_MUTE_SFT);

	return 0;
}

static int rt5639_hp_mute_put(struct snd_kcontrol *kcontrol,
		struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
...
static const struct snd_kcontrol_new rt5639_snd_controls[] = {
	...
	SOC_DOUBLE_EXT_TLV("HP Playback Volume", RT5639_HP_VOL,
		...
		rt5639_vol_rescale_get, rt5639_vol_rescale_put, out_vol_tlv),
	//bard 8-9 s
	SOC_SINGLE_TLV("Main Mic Capture Volume", RT5639_IN1_IN2,
		RT5639_BST_SFT2,  8, 0, bst_tlv), 
	SOC_SINGLE_TLV("Headset Mic Capture Volume", RT5639_IN3_IN4,
		RT5639_BST_SFT2, 8, 0, bst_tlv),
	SOC_ENUM_EXT("Main Mic Capture Switch", rt5639_mic_enum,
		rt5639_mic1_get, rt5639_mic1_put),
	SOC_ENUM_EXT("Headset Mic Capture Switch", rt5639_mic_enum,
		rt5639_mic2_get, rt5639_mic2_put),
	//bard 8-9 e
	/* OUTPUT Control */
	...
	SOC_ENUM_EXT("Playback Path", rt5639_path_type,
		rt5639_playback_path_get, rt5639_playback_path_put),
		
	SOC_ENUM_EXT("Capture MIC Path", rt5639_capture_type,
		rt5639_capture_path_get, rt5639_capture_path_put),
...
static int rt5639_do_spk_event(struct snd_soc_codec *codec, int event)
{
	DBG("%s:%d,event=%s\n",__func__,__LINE__,
		(event==SND_SOC_DAPM_POST_PMU)?"SND_SOC_DAPM_POST_PMU":"SND_SOC_DAPM_PRE_PMD");
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	rt5639_do_spk_event(codec,event);
...
{
//	static int hp_amp_power_count; //bard 6-4 r
//	printk("one bit hp_amp_power on=%d hp_amp_power_count=%d\n",on,hp_amp_power_count);

	if(on) {
		// if(hp_amp_power_count <= 0) { //bard 6-4 r
			/* depop parameters */
...
static int rt5639_do_hp_event(struct snd_soc_codec *codec, int event)
{
	DBG("%s:%d,event=%s\n",__func__,__LINE__,
		(event==SND_SOC_DAPM_POST_PMU)?"SND_SOC_DAPM_POST_PMU":"SND_SOC_DAPM_PRE_PMD");
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	rt5639_do_hp_event(codec,event);
...
static const struct snd_soc_dapm_widget rt5639_dapm_widgets[] = {
	...
	//*/
	/*
	SND_SOC_DAPM_PGA("HP amp", SND_SOC_NOPM, 
		0, 0, NULL, 0),
	SND_SOC_DAPM_PGA("SPK amp", SND_SOC_NOPM, 
		0, 0, NULL, 0),
	*/
...
/**
 * FUNCTION: 
 *	power on micbias1 for mic detect
 *
 * Author:
 *	terry_tao@asus.com
 *
 ...
 * 2013.03.21
 */
int rt5639_set_micbias1_on(bool on)
{
	struct rt5639_priv *rt5639 = NULL;
	int ret;
	if(rt3261_codec == NULL )
		return -1;
	
	rt5639 = snd_soc_codec_get_drvdata(rt3261_codec);
	...
	if(on)
	{
		ret = snd_soc_update_bits(rt3261_codec,RT5639_PWR_ANLG1,RT5639_PWR_LDO2,RT5639_PWR_LDO2);
		if(ret < 0)
			return -1;
		return snd_soc_update_bits(rt3261_codec, RT5639_PWR_ANLG2,
				RT5639_PWR_MB1 | RT5639_PWR_MB2,
				RT5639_PWR_MB1 | RT5639_PWR_MB2);
	}
	else
	{
		ret = snd_soc_update_bits(rt3261_codec,RT5639_PWR_ANLG1,RT5639_PWR_LDO2,0);
		if(ret < 0)
			return -1;
		return snd_soc_update_bits(rt3261_codec, RT5639_PWR_ANLG2,
...
EXPORT_SYMBOL(rt5639_set_micbias1_on);

/**
 * FUNCTION: 
 *	test micbias1 power status for mic detect
 *
 * Author:
 *	terry_tao@asus.com
 *
...
int rt5639_test_micbias1(void)
{
	...
	if(rt5639 == NULL)
		return -1;
	return snd_soc_test_bits(rt3261_codec, RT5639_PWR_ANLG2,
			RT5639_PWR_MB1 | RT5639_PWR_MB2,
			RT5639_PWR_MB1 | RT5639_PWR_MB2);
}
EXPORT_SYMBOL(rt5639_test_micbias1);
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_STANDBY:
		//rt5639_spk_event(codec,SND_SOC_DAPM_PRE_PMD); //bard 6-4
		//rt5639_hp_event(codec,SND_SOC_DAPM_PRE_PMD); //bard 6-4
		//terry_tao@asus.com++ keep MICBIAS1 power on
...
printk("rt5639 i2c probe initial\n");
	...
	//<-- ASUS-Bevis_chen + -->
	#ifdef CONFIG_ASUS_ENGINEER_MODE
	if(create_asusproc_codec_status_entry( ))
		printk("%s: ERROR to create codec proc entry\n",__func__);
	#endif
	//<-- ASUS-Bevis_chen - -->
