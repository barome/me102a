...
#define DDR3_DDR2_DLL_DISABLE_FREQ_ELPIDA    (200)
...
uint32_t __sramlocalfunc ddr_get_dll_disable_freq(void)
{
	uint32_t ddr_get_dll_disable_freq_value = 0;
//	ddr_print("+0x50 = %x !\n",readl_relaxed(RK30_GPIO0_BASE+0x50));
		
	if (readl_relaxed(RK30_GPIO0_BASE+0x50) & 0x10000)	//GPIO0_C0: 0 - HYNIX, 1 - ELPIDA.
	{
		ddr_get_dll_disable_freq_value = DDR3_DDR2_DLL_DISABLE_FREQ_ELPIDA;
//		ddr_print("ELPIDA !\n");
	}
	else
	{
		ddr_get_dll_disable_freq_value = DDR3_DDR2_DLL_DISABLE_FREQ;
//		ddr_print("HYNIX !\n");
	}
	return ddr_get_dll_disable_freq_value;
}
...
uint32_t ddr_get_parameter(uint32_t nMHz)
{
    ...
    if(mem_type == DDR3)
    {
        ...
        {
            p_publ_timing->mr[2] = DDR3_MR2_CWL(cwl) | (0x1<<6) /* | DDR3_Rtt_WR_60 */;
        }
    ...
    *(volatile uint32_t *)SysSrv_DdrTiming = p_noc_timing->d32;
    ...
    if(mem_type == DDR3)
    {
        ...
        pDDR_Reg->MCFG = (pDDR_Reg->MCFG & (~(0x1|(0x3<<18)|(0x1<<17)|(0x1<<16)))) | bl_tmp | tfaw_cfg(5)|pd_exit_slow|pd_type(1);
         if((ddr_freq <= ddr_get_dll_disable_freq()) && ddr_soc_is_rk3188_plus)
        {
...
uint32_t __sramlocalfunc ddr_update_mr(void)
{
    ...
    {
           if(ddr_freq>ddr_get_dll_disable_freq())
        {
            if(dll_off)  // off -> on
            {
...
#endif
    n= *(volatile uint32_t *)RK30_GPIO0_BASE;
    dsb();
...
void __sramlocalfunc ddr_selfrefresh_enter(uint32 nMHz)
{
    ...
    ddr_move_to_Config_state();
    pDDR_Reg->TZQCSI = 0;
    if((nMHz<=ddr_get_dll_disable_freq()) && ((mem_type == DDR3) || (mem_type == DDR2)))  // DLL disable
    {
        cs = ((pPHY_Reg->PGCR>>18) & 0xF);
        pPHY_Reg->MR[1] = (((p_publ_timing->mr[1])) | DDR3_DLL_DISABLE);
...
int ddr_init(uint32_t dram_speed_bin, uint32_t freq)
{
    ...
    ddr_print("ddr_get_dll_disable_freq = %d \n",ddr_get_dll_disable_freq());
