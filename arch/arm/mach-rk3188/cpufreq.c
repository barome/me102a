...
static unsigned int low_battery_freq = 1200 * 1000;
...
#ifdef CONFIG_THERMAL_US5004
extern int report_cur_temperature(void);
#endif
/*******************************************************/
...
/**********************thermal limit**************************/
//#define DEBUG_TEMP_LIMT_INFO

#define CONFIG_RK30_GPU_FREQ_LIMIT_BY_TEMP

#ifdef CONFIG_RK30_GPU_FREQ_LIMIT_BY_TEMP
static unsigned int gpu_max_freq = 600 * 1000;

static struct cpufreq_frequency_table gpu_freq_limit[] = {
	{.frequency = 600 * 1000, .index =  0},
	//{.frequency = 400 * 1000, .index = 65},
	//{.frequency = 300 * 1000, .index = 70},
	//{.frequency = 266 * 1000, .index = 75},
};
#endif
...
{ // 1 CPU busy	
	...
	{.frequency =1200 * 1000, .index =80}, 
},{ // 2 CPUs busy	
	{.frequency = 1800 * 1000, .index = 50},	
	{.frequency = 1608 * 1000, .index = 60},	
	{.frequency = 1416 * 1000, .index = 70},	
	{.frequency = 1200 * 1000, .index = 80}, 
},{ // 3 CPUs busy	
	{.frequency = 1608 * 1000, .index = 60},	
	{.frequency = 1416 * 1000, .index =70},	
	{.frequency = 1200 * 1000, .index = 75},	
	{.frequency = 1008 * 1000, .index =80}, 
},{ // 4 CPUs busy	
	{.frequency = 1416 * 1000, .index = 65},	
	{.frequency = 1200 * 1000, .index = 75},	
	{.frequency = 1008 * 1000, .index =80},	
	{.frequency = 816 * 1000, .index =85}, 
	}
};
...
#ifdef DEBUG_TEMP_LIMT_INFO
{
	static char last_sys_state = 0;
	...
	if (last_sys_state != sys_state) {
		printk("sys_state = %c\n", sys_state);
		last_sys_state = sys_state;
	}

	if (last_temp != temp) {
		printk("temperature = %d, sys_state = %c\n", temp, sys_state);
		last_temp = temp;
	}
...
#ifdef DEBUG_TEMP_LIMT_INFO
{
	static const struct cpufreq_frequency_table *last_limits_table = NULL;

	if (last_limits_table != limits_table) {
		if (limits_table == temp_limits_cpu_perf) {
			printk("limits_table = temp_limits_cpu_perf, temp = %d, sys_state = %c\n", temp, sys_state);
		} else if (limits_table == temp_limits_gpu_perf) {
			printk("limits_table = temp_limits_gpu_perf, temp = %d, sys_state = %c\n", temp, sys_state);
		} else {
			printk("limits_table = temp_limits[%d], temp = %d, sys_state = %c\n",
							(limits_table - temp_limits[0])/limits_size, temp, sys_state);
		}
		last_limits_table = limits_table;
	}
}
#endif
...
#ifdef DEBUG_TEMP_LIMT_INFO
		printk("temp limit %7d KHz cur %7d KHz, temp = %d, sys_state = %c\n", temp_limit_freq, cur_freq, temp, sys_state);
#endif
...
#ifdef CONFIG_RK30_GPU_FREQ_LIMIT_BY_TEMP
{
	const struct cpufreq_frequency_table *gpu_limits_table = gpu_freq_limit;
	size_t gpu_limits_size = ARRAY_SIZE(gpu_freq_limit);
	unsigned int gpu_new_freq = -1;

	for (i = 0; i < gpu_limits_size; i++) {
		if (temp >= gpu_limits_table[i].index) {
			gpu_new_freq = gpu_limits_table[i].frequency;
		}
	}

	if (gpu_max_freq != gpu_new_freq) {
		gpu_max_freq = gpu_new_freq;

#ifdef DEBUG_TEMP_LIMT_INFO
		printk("gpu max freq limit %7d KHz, temp = %d, sys_state = %c\n", gpu_max_freq, temp, sys_state);
#endif
		if (gpu_is_mali400) {
			if (!IS_ERR(gpu_clk)) {
				dvfs_clk_enable_limit(gpu_clk, 133000000, gpu_max_freq * 1000);
			}
		}
	}
}
...
static unsigned int cpufreq_scale_limit(unsigned int target_freq, struct cpufreq_policy *policy, bool is_private)
{
	...
	if (target_freq > low_battery_freq &&
			   rk_get_system_battery_capacity() <= low_battery_capacity) {
			target_freq = low_battery_freq;
		}
