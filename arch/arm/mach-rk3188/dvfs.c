...
static struct lkg_maxvolt lkg_volt_table[] = {
	...
	{.leakage_level = 4,	.maxvolt = 1300 * 1000},
...
void dvfs_adjust_table_lmtvolt(struct clk *clk, struct cpufreq_frequency_table *table)
{
	...
	for (i = 0; table[i].frequency != CPUFREQ_TABLE_END; i++) {
		...
		} else if (strcmp(clk->name, "core") == 0 && leakage_level <= 2 && table[i].frequency <= 1200 * KHZ) {
			printk("\t\tadjust table freq=%d KHz, index=%d mV", table[i].frequency, table[i].index);
			table[i].index += 25000;
			printk(" to index=%d mV\n", table[i].index);
		}
...
int dvfs_target(struct clk *clk, unsigned long rate_hz)
{
	...
	if (strncmp(dvfs_clk->name, "cpu", strlen("cpu") != 0)) {
		/* if up the rate */
		if (rate_new > rate_old) {
		...
		if (rate_new < rate_old) {
			if (dvfs_clk->clk_dvfs_target) {
				ret = dvfs_clk->clk_dvfs_target(clk, DVFS_TEMP_FREQ, clk_set_rate_locked);
				DVFS_DBG("dvfs %s set rate DVFS_TEMP_FREQ %lu\n", dvfs_clk->name, DVFS_TEMP_FREQ);
         //                       printk("set clk_rate tmp:312M\n");
			} else {
				ret = clk_set_rate_locked(clk, DVFS_TEMP_FREQ);
			}
		        if (ret < 0) {
                                DVFS_ERR("%s set rate err\n", __func__);
                                goto fail_roll_back;
