...
static inline void __rk_pwm_setup(const void __iomem *base, enum pwm_div div, u32 hrc, u32 lrc)
{
	...
	u32 temp = 0;
	u32 divh_old=0;
	divh_old = readl_relaxed(base + PWM_REG_HRC);
	//printk("%s,req hrc=%d,divh_old is=%d\n",__FUNCTION__,hrc,divh_old);

	if(((int)hrc-(int)divh_old >= lrc>>1)||((int)divh_old-(int)hrc >= lrc>>1))
	{
		temp = readl_relaxed(base + PWM_REG_CNTR);
		barrier();
		...
		if(temp < lrc)
			writel_relaxed(temp, base + PWM_REG_CNTR);
		else writel_relaxed(0, base + PWM_REG_CNTR);
		dsb();
		writel_relaxed(on, base + PWM_REG_CTRL);
		dsb();
	}else{

	if((int)hrc-(int)divh_old >= PWM_step)
	{
		for(divh_old += PWM_step;(int)divh_old < (int)hrc;)
		{
			temp = readl_relaxed(base + PWM_REG_CNTR);
			barrier();
			writel_relaxed(off, base + PWM_REG_CTRL);
			dsb();
			writel_relaxed(divh_old, base + PWM_REG_HRC);
			writel_relaxed(lrc, base + PWM_REG_LRC);
			if(temp < lrc)
				writel_relaxed(temp, base + PWM_REG_CNTR);
			else writel_relaxed(0, base + PWM_REG_CNTR);
		...
		writel_relaxed(on, base + PWM_REG_CTRL);
		dsb();
	}else if((int)divh_old-(int)hrc >= PWM_step)
	{
		for(divh_old -= PWM_step;(int)divh_old > (int)hrc;)
		{
			temp = readl_relaxed(base + PWM_REG_CNTR);
